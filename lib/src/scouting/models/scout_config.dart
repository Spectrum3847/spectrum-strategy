...
int? storedCounterValue(Object? stored) {
  final value = stored is num
      ? stored
      : num.tryParse(stored?.toString().trim() ?? '');
...
class ScoutConfigField {
  ...
  final String visibleWhenCode;

  final String visibleWhenValue;

  bool isVisible(Map<String, dynamic> values) =>
      visibleWhenCode.isEmpty ||
      (values[visibleWhenCode]?.toString() ?? '') == visibleWhenValue;
  ...
  ScoutConfigField copyWith({
    ...
    return ScoutConfigField(
      ...
      visibleWhenCode: visibleWhenCode,
  ...
  Map<String, dynamic> toJson() {
    return <String, dynamic>{
      ...
      if (referenceImageAsset.isNotEmpty)
        'referenceImageAsset': referenceImageAsset,
      if (visibleWhenCode.isNotEmpty) 'visibleWhenCode': visibleWhenCode,
      if (visibleWhenValue.isNotEmpty) 'visibleWhenValue': visibleWhenValue,
      if (min != null) 'min': min,
      if (max != null) 'max': max,
  ...
  factory ScoutConfigField.fromJson(Map<String, dynamic> json) {
    ...
    return ScoutConfigField(
      ...
      referenceImageAsset: json['referenceImageAsset'] as String? ?? '',
      visibleWhenCode: json['visibleWhenCode'] as String? ?? '',
      visibleWhenValue: json['visibleWhenValue'] as String? ?? '',
    );
  }
}
