...
class ScoutShiftSchedule {
  ...
  bool get hasRotationShifts => rotations.any((r) => r.shifts.isNotEmpty);
  ...
  static ScoutShiftSchedule blank({
    ...
    return ScoutShiftSchedule(
      ...
      rotations: List<ScouterShiftRotation>.unmodifiable([
        for (final entry in roster)
          ScouterShiftRotation(
            uid: entry.uid,
            name: entry.name,
            shifts: const <ScoutShiftBlock>[],
          ),
      ]),
      authorUid: authorUid,
  ...
  static ScoutShiftSchedule generate({
    ...
    String authorUid = '',
    String authorDisplayName = '',
    List<ScheduleCellEdit> cellOverrides = const <ScheduleCellEdit>[],
  }) {
    if (matchCount > kMaxMatchCount) {
      throw ArgumentError.value(
        matchCount,
    ...
    final redByColumn = _redMatchesByColumn(cellOverrides);
    return ScoutShiftSchedule(
      eventKey: eventKey,
      matchCount: matchCount < 0 ? 0 : matchCount,
      roster: List<ScoutShiftRosterEntry>.unmodifiable(roster),
      rotations: redByColumn.isEmpty
          ? _buildRotations(matchCount: matchCount, roster: roster)
          : _buildRotationsAroundRed(
              matchCount: matchCount,
              roster: roster,
              redByColumn: redByColumn,
            ),
      cellOverrides: cellOverrides,
      authorUid: authorUid,
      authorDisplayName: authorDisplayName,
  ...
  static Map<int, Set<int>> _redMatchesByColumn(
    List<ScheduleCellEdit> cellOverrides,
  ) {
    final result = <int, Set<int>>{};
    for (final edit in cellOverrides) {
      if (edit.color != ScheduleCellColor.red) continue;
      result.putIfAbsent(edit.col, () => <int>{}).add(edit.match);
    }
    return result;
  }

  static bool _isRedInRange(Set<int>? redMatches, int start, int end) {
    if (redMatches == null || redMatches.isEmpty) return false;
    for (var match = start; match <= end; match++) {
      if (redMatches.contains(match)) return true;
    }
    return false;
  }

  static List<ScouterShiftRotation> _buildRotationsAroundRed({
    required int matchCount,
    required List<ScoutShiftRosterEntry> roster,
    required Map<int, Set<int>> redByColumn,
  }) {
    ...
    for (var round = 0; round < fullRounds; round++) {
      ...
      final available =
          [
            for (var col = 0; col < roster.length; col++)
              if (!_isRedInRange(redByColumn[col], start, end)) col,
          ]..sort((a, b) {
            final breakA = start - 1 - lastEnd[a];
            final breakB = start - 1 - lastEnd[b];
            if (breakA != breakB) return breakB.compareTo(breakA);
            return a.compareTo(b);
          });
      final chosen = available.take(kShiftLength).toList(growable: false);
      for (final col in chosen) {
        shiftsByIndex[col].add(
    ...
    if (remainder > 0) {
      if (fullRounds > 0) {
        for (final col in lastRound) {
          final last = shiftsByIndex[col].removeLast();
          shiftsByIndex[col].add(
        ...
        final available = [
          for (var col = 0; col < roster.length; col++)
            if (!_isRedInRange(redByColumn[col], 1, matchCount)) col,
        ];
        for (final col in available.take(kShiftLength)) {
          shiftsByIndex[col].add(
