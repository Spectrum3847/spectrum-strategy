...
import '../services/accuracy_mapping_service.dart';

class AccuracyMappingSection extends StatefulWidget {
  const AccuracyMappingSection({this.service, super.key});

  final AccuracyMappingService? service;
...
class _AccuracyMappingSectionState extends State<AccuracyMappingSection> {
  ...
  late final AccuracyMappingService _service =
      widget.service ?? FirestoreAccuracyMappingService();
  ...
  Future<void> _load() async {
    ...
    try {
      final mapping = await _service.load();
      if (!mounted) return;
      setState(() {
        _loading = false;
        _mapping = mapping;
