...
import '../services/document_attachment_actions.dart';
...
class ScoutFormSection extends StatelessWidget {
  const ScoutFormSection({
    ...
    this.pickDocument = pickDocumentFile,
    this.openDocument = openDocumentBytes,
    super.key,
  });

  final PitPhotoUploadService? documentUploader;
  ...
  Widget _buildField(BuildContext context, ScoutConfigField field) {
    ...
    return Column(
      ...
      children: [
        ...
        if (field.referenceImageAsset.isNotEmpty) ...[
          const SizedBox(height: 8),
          ClipRRect(
            borderRadius: BorderRadius.circular(4),
            child: Image.asset(field.referenceImageAsset, fit: BoxFit.contain),
          ),
        ],
        const SizedBox(height: 8),
  ...
  Widget _buildFieldInput(
    ...
    switch (field.type) {
      ...
      case ScoutFieldType.multiCounter:
        final parsed = storedCounterValue(value);
        final intValue = parsed ?? 0;

        if (parsed != null && value is! int) values[field.code] = parsed;
      ...
      case ScoutFieldType.document:
        ...
        if (uploader == null) {
          return Text(
            'Attachments are not available in this form.',
            style: Theme.of(context).textTheme.bodySmall,
          );
        }
        return _DocumentField(
          key: ValueKey<String>('$keyPrefix-${field.code}'),
          workerKey: value?.toString() ?? '',
          uploader: uploader,
          pickDocument: pickDocument,
          openDocument: openDocument,
          onChanged: (key) => onFieldChanged(field.code, key),
        );
    }
  }
}
...
class _DocumentFieldState extends State<_DocumentField> {
  ...
  bool get _hasAttachment => widget.workerKey.isNotEmpty;
  ...
  Future<void> _pickAndUpload() async {
    ...
    final previousKey = widget.workerKey;
    try {
      ...
      final key = await widget.uploader.upload(
        picked.bytes,
      ...
      if (key == null) {
        setState(() {
          _busy = false;
          _error = 'Could not upload. Check your connection and try again.';
        });
        return;
      }
      ...
      if (previousKey.isNotEmpty && previousKey != key) {
        unawaited(widget.uploader.delete(previousKey));
      }
    } catch (e) {
      if (!mounted) return;
      setState(() {
        _busy = false;
        _error = 'Could not upload: $e';
      });
    }
  }
  ...
  Future<void> _open() async {
    ...
    try {
      final Uint8List? bytes = await widget.uploader.download(widget.workerKey);
      if (!mounted) return;
      if (bytes == null) {
        setState(() {
          _busy = false;
  ...
  @override
  Widget build(BuildContext context) {
    final ext = widget.workerKey.contains('.')
        ? widget.workerKey.split('.').last.toUpperCase()
        : '';
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
