...
import '../state/pit_shift_mirror_controller.dart';
...
class ScoutShiftScreen extends StatefulWidget {
  ...
  final PitShiftMirrorController? pitMirrorController;
...
class _ScoutShiftScreenState extends State<ScoutShiftScreen> {
  ...
  final Set<String> _locallyResolvedTradeIds = {};

  Map<String, ShiftTradeStatus> _lastTradeStatus = {};
  ...
  void initState() {
    ...
    widget.tradeController?.addListener(_onTradeStatusChanged);
  ...
  void _onEventChanged() {
    ...
    _lastTradeStatus = {};
    _locallyResolvedTradeIds.clear();
    _controller.watchEvent(key);
    widget.tradeController?.watchEvent(key);
    widget.pitMirrorController?.watchEvent(key);
  }

  @override
  void dispose() {
    widget.eventController.removeListener(_onEventChanged);
    widget.tradeController?.removeListener(_onTradeStatusChanged);
  ...
  void _onTradeStatusChanged() {
    final tradeController = widget.tradeController;
    final uid = _uid;
    if (tradeController == null || uid == null) return;
    final next = <String, ShiftTradeStatus>{};
    for (final trade in tradeController.tradesFor(uid)) {
      next[trade.id] = trade.status;
      final justResolved =
          _lastTradeStatus[trade.id] == ShiftTradeStatus.pending &&
          trade.status != ShiftTradeStatus.pending;
      final resolvedLocally = _locallyResolvedTradeIds.remove(trade.id);
      if (justResolved && !resolvedLocally) {
        _announceRemoteResolution(trade, uid);
      }
    }
    _lastTradeStatus = next;
  }

  void _announceRemoteResolution(ShiftTrade trade, String uid) {
    if (!mounted) return;
    ...
    final isRequester = uid == trade.requesterUid;
    final counterpartyName = isRequester
        ? trade.targetDisplayName
        : trade.requesterDisplayName;
    final name = counterpartyName.isEmpty ? '(no name)' : counterpartyName;
    final String message;
    if (trade.status == ShiftTradeStatus.cancelled) {
      message = isRequester
          ? 'Trade request for $rangeLabel cancelled.'
          : '$name cancelled the trade request for $rangeLabel.';
    } else if (isRequester) {
      message = trade.status == ShiftTradeStatus.accepted
          ? '$name accepted your trade request -- your shifts updated.'
          : '$name declined your trade request.';
    } else {
      message = trade.status == ShiftTradeStatus.accepted
          ? 'You accepted the trade request from $name -- your shifts '
                'updated.'
          : 'You declined the trade request from $name.';
    }
  ...
  Future<void> _openInputNamesDialog() async {
    ...
    if (schedule != null && schedule.roster.isNotEmpty) {
    ...
    final formKey = GlobalKey<_InputNamesDialogState>();
    final result = await showGlassConfirmDialog<_InputNamesResult>(
      context: context,
      title: 'Input names',
      content: _InputNamesDialog(
        key: formKey,
    ...
    await _controller.inputNames(matchCount: result.matchCount, roster: roster);
    if (!mounted) return;
  ...
  Future<void> _generateSchedule() async {
    ...
    if (schedule.hasRotationShifts) {
      final confirmed = await _confirmReplace(
        content: const Text(
          'This replaces the on-duty shifts for everyone on the roster. '
          'Red cells and notes are kept. Anyone relying on their existing '
          'shifts will see the new ones instead.',
        ),
      );
      if (confirmed != true || !mounted) return;
    }
    await _controller.generateSchedule();
  }

  Future<bool?> _confirmReplace({required Widget content}) {
    return showGlassConfirmDialog<bool>(
      context: context,
      title: 'Regenerate the shift rotation?',
      content: content,
      actionsBuilder: (dialogContext) => [
        TextButton(
        ...
        builder: (context, _) {
          ...
          if (rawSchedule == null || rawSchedule.isEmpty) {
            return Column(
              ...
              children: [
                if (pitMirror != null && !pitMirror.isEmpty)
                  Center(
                    child: ConstrainedBox(
                      constraints: const BoxConstraints(maxWidth: 720),
                      child: Padding(
                        padding: const EdgeInsets.fromLTRB(12, 12, 12, 0),
                        child: _PitScheduleSection(
                          mirror: pitMirror,
                          currentUid: _uid,
                        ),
                      ),
                ...
                Expanded(
                  child: _Empty(
                    message: _canGenerate
                        ...
                        : 'No schedule yet. Ask a strategy lead to generate '
                              'one.',
                    action: _canGenerate
                        ? FilledButton.icon(
                      ...
                      children: [
                        ...
                        if (myTrades.isNotEmpty) ...[
                          _TradeRequestsSection(
                            ...
                            currentUid: _uid!,
                            onAccept: (trade) {
                              _locallyResolvedTradeIds.add(trade.id);
                              _resolveTrade(
                                trade,
                      ...
                      Wrap(
                        ...
                        children: [
                          ...
                          FilledButton.icon(
                            ...
                            label: const Text('Generate scouting schedule'),
                          ),
                        ],
                      ),
                  ],
              ...
              Expanded(
                child: Padding(
                  ...
                  child: ScoutShiftGrid(
                    ...
                    onCellsEdit: (cells, text, color) => _controller.editCells(
                      cells: cells,
                      text: text,
                      color: color,
                    ),
...
class _PitScheduleSection extends StatelessWidget {
  const _PitScheduleSection({required this.mirror, required this.currentUid});

  final PitShiftMirror mirror;
  ...
  Widget build(BuildContext context) {
    ...
    final mineCount = uid == null ? 0 : mirror.shiftsFor(uid).length;
    final total = mirror.shifts.length;
    final summary = [
      '$total shift${total == 1 ? '' : 's'}',
      if (uid != null) '$mineCount yours',
      'synced ${_syncedText(mirror.syncedAt.toLocal())}',
    ].join(', ');

    return Card(
      margin: EdgeInsets.zero,
      child: ExpansionTile(
        key: const ValueKey('pit_schedule_section'),
        leading: const Icon(Icons.build_outlined),
        title: Text('Pit schedule', style: textTheme.titleMedium),
        subtitle: Text(summary, style: textTheme.bodySmall),
        children: [
          ConstrainedBox(
            constraints: const BoxConstraints(maxHeight: _maxListHeight),
            child: ListView.builder(
              key: const ValueKey('pit_schedule_list'),
              shrinkWrap: true,
              padding: const EdgeInsets.fromLTRB(16, 0, 16, 12),
              itemCount: mirror.shifts.length,
              itemBuilder: (context, index) {
                final shift = mirror.shifts[index];
                return _PitShiftRow(
                  shift: shift,
                  highlight: uid != null && shift.includes(uid),
                );
              },
            ),
          ),
  ...
  static String _syncedText(DateTime t) {
    const months = [
      'Jan',
      'Feb',
      'Mar',
    ...
    return '${months[t.month - 1]} ${t.day}, $hour:$minute $suffix';
  }
}

class _PitShiftRow extends StatelessWidget {
  const _PitShiftRow({required this.shift, required this.highlight});

  final MirroredPitShift shift;
  final bool highlight;
  ...
  Widget build(BuildContext context) {
    ...
    final textTheme = Theme.of(context).textTheme;
    final range = shift.rangeText();
    final names = shift.assignees.map((a) => a.name).join(', ');
    return Container(
      margin: const EdgeInsets.only(bottom: 8),
        ...
        children: [
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  shift.label.isEmpty ? shift.kind : shift.label,
                  style: textTheme.titleSmall?.copyWith(
                    fontWeight: FontWeight.w700,
                  ),
                ),
                if (names.isNotEmpty)
                  Text(names, style: textTheme.bodySmall)
                else
          ...
          if (range.isNotEmpty) ...[
            const SizedBox(width: 12),
            Text(range, style: textTheme.bodyMedium),
          ],
        ],
      ),
    );
