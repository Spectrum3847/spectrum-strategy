...
class DesktopShiftTradeSyncService implements ShiftTradeSyncService {
  ...
  Future<void> _fetchOnce() async {
    ...
    try {
      ...
      for (final doc in docs) {
        ...
        if (ts is DateTime) {
          final utc = ts.toUtc();
          if (nextCursor == null || utc.isAfter(nextCursor)) nextCursor = utc;
        }
      }
  ...
  void _applyLocalWrite(ShiftTrade trade, int token) {
    if (!_inFlightWrites.isOpen(trade.id, token)) return;
    _cache[trade.id] = trade;
    _inFlightWrites.recordPush(trade.id, trade, token);
  ...
  Future<void> create(ShiftTrade trade) async {
    ...
    try {
      ...
      _applyLocalWrite(stamped, token);
    } finally {
