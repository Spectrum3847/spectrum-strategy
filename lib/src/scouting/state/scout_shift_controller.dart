...
class ScoutShiftController extends ChangeNotifier {
  ...
  Future<void> generate({
    ...
    return _apply(generated);
  }

  Future<void> inputNames({
    required int matchCount,
    required List<ScoutShiftRosterEntry> roster,
  }) {
    if (_eventKey.isEmpty) return Future<void>.value();
  ...
  Future<void> generateSchedule() {
    ...
    final generated = ScoutShiftSchedule.generate(
      ...
      authorUid: authorUid.isEmpty ? current.authorUid : authorUid,
      authorDisplayName: authorDisplayName.isEmpty
          ? current.authorDisplayName
          : authorDisplayName,
      cellOverrides: current.cellOverrides,
    );
    return _apply(generated);
  }
  ...
  Future<void> editCells({
    required List<(int col, int match)> cells,
    required String text,
    required ScheduleCellColor? color,
  }) {
    final current = _schedule;
    if (current == null || cells.isEmpty) return Future<void>.value();
    var updated = current;
    for (final (col, match) in cells) {
      updated = updated.withCellEdit(
        col: col,
        match: match,
        text: text,
        color: color,
