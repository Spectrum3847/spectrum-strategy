...
enum RatingsSource { match13, statbotics, none }
...
class EventController extends ChangeNotifier {
  ...
  final Match13RatingsService? match13;
  ...
  RatingsSource _ratingsSource = RatingsSource.none;
  ...
  RatingsSource get ratingsSource => _ratingsSource;
  ...
  Future<void> loadEventsForYear(int year) async {
    ...
    if (_availableEvents.isEmpty) {
      ...
      if (generation != _eventsGeneration) return;
    ...
    try {
      final events = await _withOffseasonEvents(
        year,
      ...
      if (recovered) {
        ...
        _eventsError =
            'Could not load the event list (${_statboticsCauseOf(e)}).'
            '${_tbaKeyHint()}';
      }
    } finally {
  ...
  bool _superseded(int generation, String requestKey) =>
      generation != _fetchGeneration || _eventKey != requestKey;

  Future<void> _fetch() async {
    if (_eventKey.isEmpty) return;
    final requestKey = _eventKey;
    ...
    if (_teamEvents.isEmpty && _matches.isEmpty && _teamNicknames.isEmpty) {
      final cached = await _cache.loadEventData(requestKey);
      if (_superseded(generation, requestKey)) return;
      if (cached != null &&
          (cached.teamEvents.isNotEmpty ||
              cached.matches.isNotEmpty ||
              cached.teamNicknames.isNotEmpty)) {
        ...
        _dataNotice =
            'Showing cached event data from ${_ago(cached.fetchedAt)} '
            'while the latest loads.';
      }
    }
    notifyListeners();
    ...
    Future<T> guarded<T>(String part, Future<T> Function() run, T fallback) {
      return run().catchError((Object e) {
        failures[part] = _statboticsCauseOf(e);
        debugPrint('EventController: $part failed — $e');
        return fallback;
      });
    }

    final match13Ratings = match13?.ratingsFor(requestKey);
    ...
    if (_superseded(generation, requestKey)) {
      return;
    }
    ...
    final ratings = await match13Ratings;
    if (_superseded(generation, requestKey)) return;
    _applyMatch13Ratings(ratings);
    ...
    if (failures.isEmpty) {
      if (event == null && teamEvents.isEmpty && matches.isEmpty) {
        _dataNotice = _teamEvents.isEmpty
            ...
            : 'Statbotics has no data for this event. Team ratings are '
                  'match13 xP; the schedule still needs Statbotics or TBA.';
      } else {
        ...
        } else if (_ratingsSource == RatingsSource.match13) {
          _dataNotice =
              'Team ratings are match13 xP, not Statbotics EPA. Both are '
              'expected points added on the same scale.';
        }
  ...
  void _applyMatch13Ratings(Map<int, StatboticsEpa>? ratings) {
    if (ratings == null || ratings.isEmpty) {
      _ratingsSource = _teamEvents.isEmpty
          ? RatingsSource.none
          : RatingsSource.statbotics;
      return;
    }

    final rated = <StatboticsTeamEvent>[];
    final seen = <int>{};
    for (final team in _teamEvents) {
      seen.add(team.team);
      final epa = ratings[team.team];
      rated.add(epa == null ? team : _withEpa(team, epa));
    }

    final extras = ratings.keys.where((team) => !seen.contains(team)).toList()
      ..sort();
    for (final team in extras) {
      rated.add(
        StatboticsTeamEvent(
          team: team,
          event: _eventKey,
          eventName: _eventName,
          teamName: _teamNicknames[team] ?? '',
          year: _yearOfEventKey,
          wins: 0,
          losses: 0,
          ties: 0,
          epa: ratings[team]!,
    ...
    _teamEvents = rated;
    _ratingsSource = RatingsSource.match13;
  }

  static StatboticsTeamEvent _withEpa(
    StatboticsTeamEvent team,
    StatboticsEpa epa,
  ) => StatboticsTeamEvent(
    team: team.team,
    event: team.event,
  ...
  int get _yearOfEventKey =>
      _eventKey.length >= 4 ? int.tryParse(_eventKey.substring(0, 4)) ?? 0 : 0;
  ...
  Future<void> _saveSnapshot({bool servedFromCache = false}) async {
    ...
    final cached = await _cache.loadEventData(eventKey);
    ...
    await _cache.saveEventData(
      CachedEventData(
        eventKey: eventKey,
        eventName: eventName.isEmpty ? (cached?.eventName ?? '') : eventName,
  ...
  Future<bool> _recoverFromFailures(
    ...
    final tba = _tbaClient;
    if (tba != null) {
      List<TbaScheduleMatch>? scheduleValue;
      Object? scheduleFailure;
      List<TbaTeam>? namesValue;
      Object? namesFailure;
      TbaEvent? eventValue;
      Object? eventFailure;
      ...
      final calls = <Future<void>>[
        if (scheduleMissing)
          () async {
            try {
              scheduleValue = await tba.getEventMatches(requestKey);
            } catch (e) {
              scheduleFailure = e;
            }
          }(),
        if (namesMissing)
          () async {
            try {
              namesValue = await tba.getEventTeams(requestKey);
            } catch (e) {
              namesFailure = e;
            }
          }(),
        if (failures.containsKey('event info'))
          () async {
            try {
              eventValue = await tba.getEvent(requestKey);
            } catch (e) {
              eventFailure = e;
            }
          }(),
      ...
      if (_superseded(generation, requestKey)) return false;

      if (scheduleMissing) {
        if (scheduleFailure != null) {
          _noteTbaFailure(scheduleFailure!);
          failures['match schedule'] = _tbaCauseOf(scheduleFailure!);
          debugPrint(
            'EventController: TBA schedule fallback failed — $scheduleFailure',
          );
        } else if (scheduleValue!.isNotEmpty) {
          _matches = scheduleValue!
              .map(
                (m) => StatboticsMatch(
                  key: m.key,
                  event: requestKey,
          ...
          failures.remove('match schedule');
          scheduleEmptyOnTba = true;
        }
      }
      if (namesMissing) {
        if (namesFailure != null) {
          _noteTbaFailure(namesFailure!);
          debugPrint(
            'EventController: TBA team names fallback failed — $namesFailure',
          );
        } else if (namesValue!.isNotEmpty) {
          _teamNicknames = {
            for (final t in namesValue!)
      ...
      if (failures.containsKey('event info')) {
        if (eventFailure != null) {
          _noteTbaFailure(eventFailure!);
          failures['event info'] = _tbaCauseOf(eventFailure!);
          debugPrint(
            'EventController: TBA event info fallback failed — $eventFailure',
          );
        } else if (eventValue != null && eventValue!.name.isNotEmpty) {
          _eventName = eventValue!.name;
    ...
    if (noStatboticsCoverage) {
      final lead = _ratingsSource == RatingsSource.match13
          ? 'Statbotics has no data for this event, which is normal for '
                'an offseason event. Team ratings are match13 xP.'
          : 'Statbotics has no data for this event, which is normal for '
    ...
    if (tbaUsed) {
      final ratings = switch (_ratingsSource) {
        RatingsSource.match13 => 'team ratings are match13 xP',
        _ when cacheAge != null =>
          'EPA stats are cached from ${_ago(cacheAge)}',
        _ => 'EPA stats return when Statbotics recovers',
      };
      _dataNotice =
          'Statbotics is unavailable ($firstCause). The schedule and team '
          'names are live from The Blue Alliance; $ratings.';
    ...
    if (scheduleEmptyOnTba) {
      const scheduleNotice =
          'No match schedule has been posted for this event yet.';
      _dataNotice = _dataNotice == null
          ? scheduleNotice
          : '$_dataNotice $scheduleNotice';
    }
  ...
  static String _statboticsCauseOf(Object e) {
    ...
    if (e is TimeoutException) return 'Statbotics timed out';
    return 'network error';
  }

  static String _tbaCauseOf(Object e) {
    if (e is TbaApiKeyMissingException) return 'no TBA key configured';
    if (e is TbaApiException) {
      if (e.statusCode == 401) return 'TBA rejected the API key';
      if (e.statusCode == 404) return 'TBA has no record of this event';
      if (e.statusCode == 429) return 'TBA is rate-limiting us';
      if (e.statusCode >= 500) return 'TBA is busy (${e.statusCode})';
      return 'TBA error ${e.statusCode}';
    }
    if (e is FormatException || e is TypeError) {
      return 'unexpected data from TBA';
    }
    if (e is TimeoutException) return 'TBA timed out';
    return 'network error reaching TBA';
  }
