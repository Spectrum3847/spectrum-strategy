...
class StrategyController extends ChangeNotifier with WidgetsBindingObserver {
  ...
  Map<StrategyPhase, List<_PhaseSnapshot>> _undoStacks =
      _emptyPhaseSnapshotStacks();
  Map<StrategyPhase, List<_PhaseSnapshot>> _redoStacks =
      _emptyPhaseSnapshotStacks();

  static Map<StrategyPhase, List<_PhaseSnapshot>> _emptyPhaseSnapshotStacks() {
    return {
      for (final phase in StrategyPhase.values) phase: <_PhaseSnapshot>[],
    };
  }

  bool _eraseGroupOpen = false;
  final Set<StrategyPhase> _noteUndoGroupOpen = {};
  final Map<StrategyPhase, Timer> _noteUndoTimers = {};
  static const Duration _noteUndoCoalesceWindow = Duration(milliseconds: 500);
  ...
  void updateNote(String note) {
    ...
    if (!_noteUndoGroupOpen.contains(phase)) {
      _pushUndo(phase);
      _noteUndoGroupOpen.add(phase);
    }
    _noteUndoTimers[phase]?.cancel();
    _noteUndoTimers[phase] = Timer(_noteUndoCoalesceWindow, () {
      _noteUndoGroupOpen.remove(phase);
      _noteUndoTimers.remove(phase);
    });
    _session.notesByPhase[phase] = note;
    _touch(persist: true);
  }

  void startStroke(StrategyPoint point, {bool isLegend = false}) {
  ...
  void placeRobot(StrategyPoint point) {
    ...
    _pushUndo(_session.selectedPhase);
  ...
  bool eraseAt(Offset position, Size size, {bool isLegend = false}) {
    ...
    for (var i = markers.length - 1; !isLegend && i >= 0; i--) {
      final center = markers[i].position.toOffset(size);
      if ((position.dx - center.dx).abs() <= _markerHitHalfExtent &&
          (position.dy - center.dy).abs() <= _markerHitHalfExtent) {
        if (!_eraseGroupOpen) {
          _pushUndo(phase);
          _eraseGroupOpen = true;
        }
        markers.removeAt(i);
        _touch(persist: true);
        return true;
    ...
    for (var i = strokes.length - 1; i >= 0; i--) {
      if (strokes[i].isLegend == isLegend &&
          _strokeHitsPoint(strokes[i], position, size)) {
        if (!_eraseGroupOpen) {
          _pushUndo(phase);
          _eraseGroupOpen = true;
        }
  ...
  bool get canUndo => _undoStacks[_session.selectedPhase]!.isNotEmpty;
  bool get canRedo => _redoStacks[_session.selectedPhase]!.isNotEmpty;

  void undo() {
    final phase = _session.selectedPhase;
    final undoStack = _undoStacks[phase]!;
    ...
    _eraseGroupOpen = false;
    _noteUndoGroupOpen.remove(phase);
    _noteUndoTimers.remove(phase)?.cancel();
    final previous = undoStack.removeLast();
    _redoStacks[phase]!.add(_capturePhaseSnapshot(phase));
    _applyPhaseSnapshot(phase, previous);
    _touch(persist: true);
  }

  void redo() {
    final phase = _session.selectedPhase;
    final redoStack = _redoStacks[phase]!;
    ...
    _undoStacks[phase]!.add(_capturePhaseSnapshot(phase));
    _applyPhaseSnapshot(phase, next);
    _touch(persist: true);
  }

  void _pushUndo(StrategyPhase phase) {
    final stack = _undoStacks[phase]!;
    stack.add(_capturePhaseSnapshot(phase));
    if (stack.length > _maxUndoDepth) {
      stack.removeAt(0);
    }
    _redoStacks[phase]!.clear();
  }

  _PhaseSnapshot _capturePhaseSnapshot(StrategyPhase phase) {
    return _PhaseSnapshot(
      strokes: _session.strokesByPhase[phase]!
          .map((stroke) => StrategyStroke.fromJson(stroke.toJson()))
          .toList(),
      markers: _session.markersByPhase[phase]!
          .map((marker) => RobotMarker.fromJson(marker.toJson()))
          .toList(),
      note: _session.notesByPhase[phase] ?? '',
    );
  }

  void _applyPhaseSnapshot(StrategyPhase phase, _PhaseSnapshot snapshot) {
    _session.strokesByPhase[phase] = snapshot.strokes
        .map((stroke) => StrategyStroke.fromJson(stroke.toJson()))
        .toList();
    _session.markersByPhase[phase] = snapshot.markers
        .map((marker) => RobotMarker.fromJson(marker.toJson()))
        .toList();
    _session.notesByPhase[phase] = snapshot.note;
  }

  void _resetUndoHistory() {
    _undoStacks = _emptyPhaseSnapshotStacks();
    _redoStacks = _emptyPhaseSnapshotStacks();
    _eraseGroupOpen = false;
    _noteUndoGroupOpen.clear();
    for (final timer in _noteUndoTimers.values) {
      timer.cancel();
    }
    _noteUndoTimers.clear();
  }
...
class _PhaseSnapshot {
  ...
  final List<RobotMarker> markers;
  final String note;
}
