...
enum _RankSort { iqmScore, consistency, epa, opr, qualRank }
...
class _PlayoffRankingScreenState extends State<PlayoffRankingScreen> {
    ...
    final rows = teams
        .map((te) {
          return _RankRow(
            ...
            opr: widget.statsController?.oprFor(te.team),
            qualRank: widget.statsController?.rankFor(te.team),
          );
        })
        .toList(growable: false);

    final sorted = switch (_sort) {
      _RankSort.iqmScore => _sortByIqmScore(rows),
      _RankSort.consistency => _sortByConsistency(rows),
      _RankSort.epa => _sortByEpa(rows),
      _RankSort.opr => _sortByOpr(rows),
      _RankSort.qualRank => _sortByQualRank(rows),
    };
              ...
              child: LayoutBuilder(
                builder: (context, constraints) {
                  final list = ListView.builder(
                    key: const Key('rankingRowsList'),
                    padding: const EdgeInsets.fromLTRB(16, 8, 16, 24),
                    itemCount: sorted.length + leadingCount,
                    itemBuilder: (context, index) {
                      if (index == leadingCount - 1) {
                        return _TableHeader(sort: _sort);
                      }
                      final rank = index - leadingCount + 1;
                      final row = sorted[rank - 1];
                      return Column(
  ...
  static List<_RankRow> _sortByOpr(List<_RankRow> rows) {
    final copy = rows.toList();
    copy.sort((a, b) {
      final oa = a.opr;
      final ob = b.opr;
  ...
  static List<_RankRow> _sortByQualRank(List<_RankRow> rows) {
    final copy = rows.toList();
    copy.sort((a, b) {
      final ra = a.qualRank;
      final rb = b.qualRank;
            ...
            child: SingleChildScrollView(
              ...
              child: SegmentedButton<_RankSort>(
                ...
                segments: const [
                  ...
                  ButtonSegment(
                    value: _RankSort.opr,
                    label: SegmentLabel('OPR'),
                  ),
                  ButtonSegment(
                    value: _RankSort.qualRank,
                    label: SegmentLabel('Rank'),
                  ),
                ],
                selected: {sort},
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          SizedBox(
            width: 52,
            child: Text('OPR', style: style, textAlign: TextAlign.end),
          ),
          SizedBox(
            width: 44,
            child: Text('Rank', style: style, textAlign: TextAlign.end),
          ),
          SizedBox(
            width: 48,
          ...
          child: Row(
            children: [
              ...
              SizedBox(
                width: 52,
                child: Text(
                  row.opr != null ? row.opr!.toStringAsFixed(1) : '--',
                  textAlign: TextAlign.end,
                  style: text.bodySmall?.copyWith(
                    color: row.opr != null
                        ? scheme.onSurface
