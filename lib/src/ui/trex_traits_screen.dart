...
class _TrexTraitTabViewState extends State<_TrexTraitTabView> {
  ...
  void initState() {
    ...
    _eventCtrl.text = eventController.eventName;

    if (eventController.availableEvents.isEmpty &&
        !eventController.eventsLoading) {
      unawaited(eventController.loadEventsForYear(DateTime.now().year));
    }
  }
  ...
  Future<void> _submit() async {
    ...
    final eventName = _eventCtrl.text.trim();
    if (eventName.isEmpty) {
      setState(() {
        _statusIsError = true;
        _statusMessage = 'Enter an event.';
      });
      return;
    }
    final reportText = _reportCtrl.text.trim();
    if (reportText.isEmpty) {
      setState(() {
        _statusIsError = true;
        _statusMessage = 'Enter a report.';
      });
      return;
    }
    setState(() {
    ...
    if (saved) {
      ...
      _eventCtrl.text = widget.eventController?.eventName ?? '';
      _reportCtrl.clear();
            ...
            child: ListView(
              ...
              children: [
                  ...
                  children: [
                    ...
                    if (widget.eventKey.isNotEmpty)
                      FilterChip(
                        ...
                        selected: _showAllEvents,
                        onSelected: (value) =>
                            setState(() => _showAllEvents = value),
                      ),
                  ],
                ),
                ...
                _TrexReportDatabase(
                  ...
                  canEditAnyEntry: widget.canEditAnyEntry,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          _EventField(eventCtrl: eventCtrl, eventController: eventController),
          const SizedBox(height: 12),
          TextField(
...
class _EventField extends StatelessWidget {
  const _EventField({required this.eventCtrl, this.eventController});
  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        final names = <String>{
          for (final event in controller.availableEvents) event.name,
          if (eventCtrl.text.isNotEmpty) eventCtrl.text,
        }.toList();
        final value = names.contains(eventCtrl.text) ? eventCtrl.text : null;
        return DropdownButtonFormField<String>(
          ...
          hint: Text(controller.eventsLoading ? 'Loading events…' : 'Event'),
          items: [
            for (final name in names)
              DropdownMenuItem(value: name, child: Text(name)),
          ],
          onChanged: (selected) => eventCtrl.text = selected ?? '',
        );
      },
    );
  }
...
class _TrexReportDatabase extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final visible = eventKey.isEmpty || showAllEvents
        ? reports
        : reports
              .where((r) => r.eventKey.isEmpty || r.eventKey == eventKey)
              .toList();
    ...
    if (teams.isEmpty) {
      return EmptyState(
        ...
        message: reports.isEmpty
            ...
            : 'No reports for this event yet. Try "All events".',
      );
    }
    return Column(
      children: [
...
class _ReportCard extends StatelessWidget {
  ...
  bool _canDelete() {
    final myUid = controller.currentUserUid;

    final isOwn = report.authorUid.isNotEmpty && report.authorUid == myUid;
    return isOwn || canEditAnyEntry;
  }
  ...
  Future<void> _confirmDelete(BuildContext context) async {
    ...
    final confirmed = await showGlassConfirmDialog<bool>(
      ...
      title: 'Delete report?',
      content: Text(
        'This will remove the ${report.trait} report for team '
        '${report.teamNumber}, including from the shared database.',
      ),
    ...
    final deleted = await controller.deleteReport(report.id);
    if (deleted) return;
    messenger.showSnackBar(
      SnackBar(
        content: Text(controller.lastError ?? 'Could not delete that report.'),
      ),
    );
