...
class _TeamCompareViewState extends State<TeamCompareView> {
  ...
  static const double _minColumnWidth = 340;
        ...
        return LayoutBuilder(
          builder: (context, constraints) {
            final available =
                ...
                _padding * 2 -
                _spacing * (_columnCount - 1);
            final columnWidth = math.max(
              _minColumnWidth,
              available / _columnCount,
            );
...
class _CompareColumnBody extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final (auto, teleop, matchNumbers) = TeamCompareStats.fuelSeries(rows);
    ...
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 12),
        _FuelGraph(auto: auto, teleop: teleop, matchNumbers: matchNumbers),
        TeamCompareAiCard(
          assistant: assistant,
...
String fuelChartXLabel(int index, List<int?> matchNumbers) {
  final number = index < matchNumbers.length ? matchNumbers[index] : null;
...
class _FuelGraph extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final axisTitleStyle = text.labelSmall?.copyWith(
      ...
      children: [
        ...
        Row(
          ...
          children: [
            RotatedBox(
              quarterTurns: 3,
              child: Text('Fuel scored', style: axisTitleStyle),
            ),
            Expanded(
              child: Column(
                children: [
                  SizedBox(
                    ...
                    child: CustomPaint(
                      painter: _FuelGraphPainter(
                        ...
                        labelColor: StrategyPalette.mutedTextOf(context),
                      ),
                    ),
                  ),
                  Text('Match number', style: axisTitleStyle),
                ],
              ),
            ),
          ],
...
class _FuelGraphPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i <= 2; i++) {
      ...
      final value = maxValue - maxValue * i / 2;
      _drawLabel(
        canvas,
    ...
    for (var i = 0; i < count; i++) {
      ...
      _drawLabel(
        canvas,
        fuelChartXLabel(i, matchNumbers),
        labelStyle,
        Offset(x, bottom + 4),
  ...
  void _drawLabel(
    ...
    if (maxRight != null) {
      dx = dx.clamp(0.0, math.max(0.0, maxRight - painter.width));
    }
    final dy = centerVertically ? anchor.dy - painter.height / 2 : anchor.dy;
  ...
  bool shouldRepaint(covariant _FuelGraphPainter oldDelegate) =>
      ...
      !listEquals(matchNumbers, oldDelegate.matchNumbers) ||
