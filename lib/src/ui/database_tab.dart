...
class _DatabaseTabState extends State<DatabaseTab> {
  ...
  List<ScoutEntry> _scopedToEvent(List<ScoutEntry> all) {
    final eventKey = widget.eventController.eventKey;
    if (eventKey.isEmpty) return all;
    return all
        .where((entry) {
          final entryEventKey = eventKeyOfEntry(entry);
          return entryEventKey == null || entryEventKey == eventKey;
        })
  ...
  Future<void> _exportCsv() async {
    ...
    try {
      final message = await _exportService.shareCsv(
        config: widget.configController.config,
        entries: _scopedToEvent(widget.scoutingController.entries),
        eventKey: eventKey,
      );
      if (!mounted) return;
