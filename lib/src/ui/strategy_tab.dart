...
class StrategyTabState extends State<StrategyTab> {
  ...
  Future<void> shareBoard() async {
    try {
      final message = await _exportService.shareBoardImage(
  ...
  String _toolHint(StrategyTool tool) {
    switch (tool) {
      ...
      case StrategyTool.robot:
        return 'Robot: tap the board to drop a robot marker.';
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Expanded(
                child: FutureBuilder<FieldMapCatalogData>(
  ...
  Widget _buildToolbar(BuildContext context) {
    return HorizontalStepShortcuts(
      ...
      child: Wrap(
        ...
        children: [
          ...
          DropdownButton<String>(
            value: const ['Red', 'Blue'].contains(controller.session.alliance)
          ...
          _buildFieldPicker(context),
          Wrap(
            spacing: 8,
            runSpacing: 8,
            children: StrategyPhase.values
                .map((phase) {
                  final selected = controller.session.selectedPhase == phase;
                  return ChoiceChip(
          ...
          ...StrategyTool.values.map((tool) {
            final selected = controller.session.selectedTool == tool;
            return Tooltip(
              message: _toolHint(tool),
  ...
  Widget _buildFieldPicker(BuildContext context) {
    return FutureBuilder<FieldMapCatalogData>(
      ...
      builder: (context, snapshot) {
        ...
        final fields = snapshot.data?.fields ?? const <FieldMapDefinition>[];
        final selected = snapshot.data?.fallbackFor(
          controller.session.selectedFieldId,
        );
        if (fields.length < 2 || selected == null) {
