...
class _PitScoutingScreenState extends State<PitScoutingScreen> {
  ...
  PitScoutEntry? _targetEntry() {
    final editingOther = _entryBeingEditedByOther();
    if (editingOther != null) return editingOther;
    final teamNumber = int.tryParse(_teamNumberCtrl.text.trim()) ?? 0;
    if (teamNumber <= 0) return null;
    return _ownEntryForTeam(teamNumber);
  }

  PitScoutEntry? _entryBeingEditedByOther() {
  ...
  Future<void> _saveResolvedEntry(int teamNumber) async {
    final existing = _entryBeingEditedByOther() ?? _ownEntryForTeam(teamNumber);
  ...
  Widget _buildSyncStatus() {
    ...
    final (String label, IconData icon) = switch (status.state) {
      ...
      PitScoutingSyncState.rejected => (
        status.error != null ? 'Not accepted: ${status.error}' : 'Not accepted',
        Icons.report_gmailerrorred_rounded,
      ),
    };
    final isRejected = status.state == PitScoutingSyncState.rejected;
  ...
  Widget _buildEditingNotice() {
    ...
    if (editingOther == null) {
      message = 'Editing your saved entry for this team. Saving updates it.';
    } else {
      final author = editingOther.authorDisplayName.isNotEmpty
          ? editingOther.authorDisplayName
          : 'another scouter';
      message =
          "Editing $author's saved entry for this team. Saving updates it; "
  ...
  Widget _buildEntryTile(PitScoutEntry entry) {
    ...
    final canManage = isOwn || widget.canEditAnyEntry;
          ...
          ? Row(
              ...
              children: [
                IconButton(
                  ...
                  tooltip: 'Edit entry',
                  onPressed: _saving ? null : () => _startEditing(entry),
                ),
                IconButton(
  ...
  void _startEditing(PitScoutEntry entry) {
    if (_saving) return;
