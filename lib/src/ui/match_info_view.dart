...
class MatchInfoView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final prescouting = prescoutingController;
    if (prescouting != null) listenables.add(prescouting);
    final board = playoffBoardController;
    if (board != null) listenables.add(board);
    final postMatch = postMatchReportController;
    if (postMatch != null) listenables.add(postMatch);
    ...
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        ...
        final entries = MatchInfoStats.build(
          ...
          prescoutEntries: prescouting?.entries ?? const [],
        );
          ...
          children: [
            ...
            LayoutBuilder(
              builder: (context, constraints) {
                ...
                final postMatch = controller == null
                    ? null
                    : _PostMatchTable(
                        controller: controller,
                        eventKey: eventKey,
                        matchId: entry.match.key,
                        canEdit: canEditPostMatch,
                      );
...
String _fmtSourced(SourcedStat stat) {
  final event = stat.event;
  final prescout = stat.prescout;
  if (event == null && prescout == null) return '--';
  if (prescout == null) return formatStat(event!);
  if (event == null) return '${formatStat(prescout)} (prescout)';
  return '${formatStat(event)} (event), ${formatStat(prescout)} (prescout)';
}
...
final List<MatchInfoColumn> matchInfoColumns = <MatchInfoColumn>[
  ...
  MatchInfoColumn(
    label: 'Max auto',
    code: 'maxAuto',
    read: (row) => _fmtSourced(row.maxAuto),
  ),
  MatchInfoColumn(
    label: 'IQM auto',
    code: 'iqmAuto',
    read: (row) => _fmtSourced(row.iqmAuto),
  ),
  MatchInfoColumn(
    label: 'Teleop (avg)',
    code: 'teleop',
    read: (row) => _fmtSourced(row.teleopAverage),
  ),
  MatchInfoColumn(
    label: 'IQM fuel',
    code: 'iqmFuel',
    read: (row) => _fmtSourced(row.iqmFuel),
  ),
  MatchInfoColumn(
    label: 'Max teleop',
    code: 'maxTeleop',
    read: (row) => _fmtSourced(row.maxTeleop),
  ),
  MatchInfoColumn(
    label: 'Auto climb',
    code: 'autoClimb',
    read: (row) => row.autoClimb ? 'Yes' : '',
  ),
  MatchInfoColumn(
    label: 'Endgame climb',
    code: 'endgameClimb',
    read: (row) => row.endgameClimb,
  ),
  MatchInfoColumn(label: 'Notes', code: 'notes', read: (row) => row.notes),
  MatchInfoColumn(
    label: 'Drive coach',
    code: 'driveCoach',
    read: (row) => row.driveCoach,
  ),
];
...
class _PostMatchTable extends StatefulWidget {
  const _PostMatchTable({
  ...
  @override
  State<_PostMatchTable> createState() => _PostMatchTableState();
}

class _PostMatchTableState extends State<_PostMatchTable> {
  late final TextEditingController _auto;
  late final TextEditingController _teleop;
  late final TextEditingController _endgame;
  ...
  void initState() {
    ...
    _auto = TextEditingController(text: _loaded.auto);
    _teleop = TextEditingController(text: _loaded.teleop);
    _endgame = TextEditingController(text: _loaded.endgame);
    _notes = TextEditingController(text: _loaded.notes);
  ...
  void _onControllerChanged() {
    ...
    if (latest.updatedAt == _loaded.updatedAt) return;
    setState(() {
      _loaded = latest;
      _auto.text = latest.auto;
      _teleop.text = latest.teleop;
      _endgame.text = latest.endgame;
      ...
      children: [
            ...
            if (widget.canEdit)
              IconButton(
                icon: _saving
                    ...
                    : Icon(
                        _dirty ? Icons.save_outlined : Icons.check,
                        size: 18,
                      ),
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              _postMatchField('Auton', _auto),
              const SizedBox(height: 8),
              _postMatchField('Teleop', _teleop),
              const SizedBox(height: 8),
              _postMatchField('Endgame', _endgame),
              const SizedBox(height: 8),
              _postMatchField('Notes', _notes, minLines: 4),
            ],
          ),
        ),
      ],
