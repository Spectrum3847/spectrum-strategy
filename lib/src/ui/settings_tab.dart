...
class _SettingsTabState extends State<SettingsTab> {
  ...
  Future<void> _reportProblem() async {
    ...
    String? area;
    String? impact;
    final screenshots = <PickedScreenshot>[];
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  if (_reportService.supportsScreenshots) ...[
                    const SizedBox(height: 8),
                    _ScreenshotPicker(
        ...
        return ListView(
          ...
          children: [
            ...
            if (_canEditAccuracyMapping && _firebaseReady) ...[
              const SizedBox(height: 24),
              AccuracyMappingSection(service: widget.accuracyMappingService),
            ],
            const SizedBox(height: 24),
            Text(
              'Pit Scouting Form',
            ...
            Card(
              child: ListTile(
                ...
                title: const Text('About Spectrum Strategy'),
                subtitle: const Text(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 4),
            Text(
              'Off by default: a launch can land mid-event, between matches, '
              'with unsaved data on screen. Turn this on to check for a '
              'newer release on launch and install it without asking, or '
              'leave it off and check and download by hand below.',
              style: Theme.of(context).textTheme.bodySmall,
            ),
            const SizedBox(height: 4),
...
class _MinimizeToTrayTile extends StatefulWidget {
  const _MinimizeToTrayTile();

  @override
  State<_MinimizeToTrayTile> createState() => _MinimizeToTrayTileState();
}

class _MinimizeToTrayTileState extends State<_MinimizeToTrayTile> {
  final DesktopTrayService _service = DesktopTrayService.shared;
  bool _enabled = true;
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 4),
            Text(
              'Keep the app running in the tray when the window is '
              'minimized or closed, so this computer can keep answering '
              'the team\'s AI requests.',
              style: Theme.of(context).textTheme.bodySmall,
            ),
            SwitchListTile(
              contentPadding: EdgeInsets.zero,
              title: const Text('Minimize/close to the system tray'),
              value: _enabled,
              onChanged: _toggle,
            ),
          ],
...
class _ScreenshotPicker extends StatelessWidget {
  ...
  Future<void> _add() async {
    try {
      final picked = await pickReportScreenshot();
      if (picked == null) return;
      screenshots.add(picked);
      onChanged();
    } on ScreenshotPickException catch (e) {
      onError(e.message);
    } catch (e) {
