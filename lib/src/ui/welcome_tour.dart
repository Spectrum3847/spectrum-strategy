...
class WelcomeTourOverlay extends StatefulWidget {
  const WelcomeTourOverlay({
    ...
    this.menuKey,
    this.menuItems = const <TourMenuItem>[],
    this.onStepChanged,
    super.key,
  });
  ...
  final List<TourMenuItem> menuItems;
...
class _WelcomeTourOverlayState extends State<WelcomeTourOverlay> {
  ...
  bool get _currentStepHasMenuTarget {
    final tabIndex = _currentStep?.tabIndex;
    return tabIndex != null &&
        widget.menuItems.any((i) => i.tabIndex == tabIndex);
  }

  Rect? _menuButtonRect() {
    final box = widget.menuKey?.currentContext?.findRenderObject();
    if (box is! RenderBox || !box.hasSize || box.size.isEmpty) return null;
    return box.localToGlobal(Offset.zero) & box.size;
  }

  Rect? _targetRect() => _currentStepHasMenuTarget ? _menuButtonRect() : null;
  ...
  Widget build(BuildContext context) {
    ...
    final mockMenu = _targetRect();
    final target = mockMenu;
...
class _MockTabMenu extends StatelessWidget {
  const _MockTabMenu({
  ...
  Widget build(BuildContext context) {
    ...
    return Positioned(
      top: anchor.bottom + 4,
      left: anchor.left.clamp(0.0, screenWidth),
      child: Material(
        elevation: 8,
        color: StrategyPalette.surfaceOf(context),
        borderRadius: const BorderRadius.all(
