...
class PlayoffBoard {
  ...
  static const int meetingColumnCount = 6;

  static const int meetingExtraRowsLastColumn = 10;
  ...
  List<int> get meetingTeamsInReadingOrder {
    ...
    final lastColumn = meetingColumnCount - 1;
    for (
      var row = meetingRowCount;
      row < meetingRowCount + meetingExtraRowsLastColumn;
      row++
    ) {
      final team = int.tryParse(meetingCell(row, lastColumn).trim());
      if (team != null) teams.add(team);
    }
    return teams;
  }
  ...
  factory PlayoffBoard.fromJson(Map<String, dynamic> json) {
    ...
    return PlayoffBoard(
      columnLabels: labels.length == meetingColumnCount - 1
          ? <String>[...labels, defaultColumnLabels[meetingColumnCount - 1]]
          : labels.length == meetingColumnCount
          ? labels
          : defaultColumnLabels,
