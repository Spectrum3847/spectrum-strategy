...
class LocalAssistantBackend implements AssistantBackend {
  ...
  Future<AssistantModel?> _resolveModel({String? requestedId}) async {
    if (_fixedModel != null) return _fixedModel;
    final installed = await _runtime.installedModels();
    if (installed.isEmpty) return null;
    if (requestedId != null) {
      for (final model in installed) {
        if (model.id == requestedId) return model;
      }
    }
    final budget = LlamaRuntimeService.sizeBudgetBytes(
      ramGb: await _runtime.totalRamGb(),
      vramGb: await _runtime.totalVramGb(),
    );
    return LlamaRuntimeService.recommendedModel(installed, budget);
  }
  ...
  Future<AssistantSummary> complete(AssistantRequest request) async {
    final model = await _resolveModel(requestedId: request.modelId);
    if (model == null) {
      throw const AssistantUnavailable(
        'No local model installed. Download one on the AI tab, under Models.',
      );
    }
  ...
  Future<String> _ask({
    ...
    final baseMessages = <Map<String, dynamic>>[
      if (request.system != null)
        <String, String>{'role': 'system', 'content': request.system!},
      for (final turn in request.history ?? const <AssistantTurn>[])
        <String, String>{'role': turn.role.name, 'content': turn.content},
      <String, String>{'role': 'user', 'content': request.prompt},
    ];

    final registry = request.useTools ? _tools : null;
    final message = await runAssistantToolLoop(
      messages: baseMessages,
      registry: registry,
      complete: (messages, tools) => _complete(uri, model, messages, tools),
    );
    return _answerFrom(message);
  }
  ...
  Future<Map<String, dynamic>> _complete(
    ...
    if (message is! Map) {
      throw const AssistantUnavailable(
        'Local model server answered with no message.',
      );
    }
    return Map<String, dynamic>.from(message);
  }

  String _answerFrom(Map<String, dynamic> message) {
