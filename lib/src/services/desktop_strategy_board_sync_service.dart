...
class DesktopStrategyBoardSyncService implements StrategyBoardSyncService {
  ...
  Future<void> push(StrategySession session) async {
    ...
    final token = _inFlightWrites.beginPush(session.id, session);
    try {
  ...
  Future<void> delete(StrategySession session) async {
    ...
    final token = _inFlightWrites.beginDelete(session.id);
    try {
      ...
      if (_inFlightWrites.isOpen(session.id, token)) {
        _cache.remove(session.id);

        _inFlightWrites.recordDelete(session.id, token);
      }
      _emitSynced();
    } catch (error) {
