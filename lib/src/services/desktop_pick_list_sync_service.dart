...
class DesktopPickListSyncService implements PickListSyncService {
  ...
  void _onAuthChanged(SpectrumAuthSnapshot snapshot) {
    _inFlightWrites.syncSession(snapshot.user?.uid);
  ...
  Future<void> push(PickList list) async {
    ...
    final token = _inFlightWrites.beginPush(stamped.id, stamped);
    try {
      ...
      if (_inFlightWrites.isOpen(stamped.id, token)) {
        _cache[stamped.id] = stamped;

        _inFlightWrites.recordPush(stamped.id, stamped, token);
  ...
  Future<void> _pushTeamOp(
    ...
    final cachedBefore = _cache[list.id];
    final token = _inFlightWrites.beginPush(
      list.id,
      cachedBefore == null
          ? list
          : _withTeamOp(
    ...
    try {
      ...
      if (_inFlightWrites.isOpen(list.id, token)) {
        final cached = _cache[list.id];
        if (cached != null) {
          final updated = _withTeamOp(
            cached,
            team: team,
            isAdd: isAdd,
            updatedAt: list.updatedAt,
          ...
          _inFlightWrites.recordPush(list.id, updated, token);
        }
  ...
  static PickList _withTeamOp(
    ...
    final teams = isAdd
        ? (list.teamNumbers.contains(team)
              ? list.teamNumbers
              : [...list.teamNumbers, team])
        : list.teamNumbers.where((t) => t != team).toList();
    return list.copyWith(teamNumbers: teams, updatedAt: updatedAt);
  }
  ...
  Future<void> _deleteById(String id) async {
    ...
    try {
      ...
      if (_inFlightWrites.isOpen(id, token)) {
        _cache.remove(id);
    ...
    try {
      ...
      for (final doc in docs) {
        ...
        if (serverTs is DateTime) {
          ...
          if (nextCursor == null || ts.isAfter(nextCursor)) {
            nextCursor = ts;
          }
        }
      }
      final resolved = _inFlightWrites.resolve(next);
      if (resolved == null) {
        return;
      }
      if (isFullSync) {
      ...
      _cursor = nextCursor ?? _clock().toUtc();
      _cache
        ..clear()
        ..addAll(resolved);
