...
class DesktopTrexTraitReportSyncService implements TrexTraitReportSyncService {
  ...
  static const String _deletedCollection = 'trexTraitReports_deleted';
  ...
  final Map<String, TrexTraitReport> _cache = <String, TrexTraitReport>{};
  ...
  @override
  Future<void> delete(TrexTraitReport report) => _deleteById(report.id);
  ...
  Future<void> _syncOnce() async {
    ...
    try {
      ...
      final next = isFullSync
          ? <String, TrexTraitReport>{}
          : Map<String, TrexTraitReport>.of(_cache);
      for (final doc in docs) {
        final decoded = _decode(doc);
        if (decoded == null) {
          continue;
        }
        next[decoded.id] = decoded;
