...
class DesktopUpdateService {
  DesktopUpdateService({
    ...
    this._assetSelector,
       ...
       _buildCommit = buildCommit ?? _envGitCommit,
  ...
  static const String _envGitCommit = String.fromEnvironment('GIT_COMMIT');

  static const String nightlyManifestAsset = 'nightly-manifest.json';

  static const int _minCommitPrefix = 7;
  ...
  Future<DesktopUpdateCheck> _checkNightly(String currentRaw) async {
    if (_buildCommit.trim().isEmpty && _isFreshNightly()) {
      return const DesktopUpdateCheck.upToDate();
    }
    ...
    final published = await _publishedNightlyCommit(release.manifestUrl);
    if (published != null && _buildCommit.trim().isNotEmpty) {
      if (_sameCommit(published, _buildCommit)) {
        return const DesktopUpdateCheck.upToDate();
      }
      return _nightlyUpdate(currentRaw, release, latestVersion: published);
    }

    if (_isFreshNightly()) {
      return const DesktopUpdateCheck.upToDate();
    }
    return _nightlyUpdate(currentRaw, release, latestVersion: release.rawTag);
  }

  DesktopUpdateCheck _nightlyUpdate(
    String currentRaw,
    _ReleaseSnapshot release, {
  ...
  Future<String?> _publishedNightlyCommit(String? manifestUrl) async {
    if (manifestUrl == null) {
      return null;
    }
    final url = Uri.tryParse(manifestUrl);
    if (url == null) {
      return null;
    }
  ...
  Future<_ReleaseSnapshot?> _loadRelease(
    ...
    return _ReleaseSnapshot(
      ...
      manifestUrl: _namedAssetUrl(decoded['assets'], nightlyManifestAsset),
    );
  }
  ...
  static String? _namedAssetUrl(dynamic assets, String name) {
