...
import 'src/services/desktop_tray_service.dart';
...
import 'src/scouting/services/accuracy_mapping_service.dart';
import 'src/scouting/services/desktop_accuracy_mapping_service.dart';
...
import 'src/scouting/services/desktop_pit_shift_mirror_sync_service.dart';
...
import 'src/scouting/services/pit_shift_mirror_sync_service.dart';
...
import 'src/scouting/state/pit_shift_mirror_controller.dart';
...
import 'src/scouting/services/pit_photo_store_web.dart'
...
import 'src/services/assistant/app_data_tool_provider.dart';
import 'src/services/assistant/assistant_cache.dart';
import 'src/services/assistant/assistant_request_worker.dart';
import 'src/services/assistant/assistant_requests.dart';
import 'src/services/assistant_chat_store.dart';
import 'src/services/remote_chat_store.dart';
import 'src/services/llama_runtime_service.dart';
...
import 'src/state/assistant_chat_controller.dart';
...
Future<void> main() async {
  ...
  if (_isDesktop) await DesktopTrayService.shared.start();
  ...
  LicenseRegistry.addLicense(() async* {
    final license = await rootBundle.loadString(
      'assets/fields/wpilib/LICENSE.txt',
    );
    yield LicenseEntryWithLineBreaks(<String>['WPILib field assets'], license);
  });
  ...
  PitShiftMirrorSyncService? pitShiftMirrorSyncService;
  FirestoreTbaConfig? teamTbaConfig;

  FirestoreApiKeyConfig? assistantConfig;
  ...
  RemoteAssistantRequests? assistantRequests;
  ...
  if (firebaseReady && !_isDesktop) {
    ...
    accuracyMappingService = FirestoreAccuracyMappingService();
    ...
    pitShiftMirrorSyncService = FirestorePitShiftMirrorSyncService(
      authService: authService,
    );
    ...
    assistantConfig = FirestoreApiKeyConfig();
    huggingFaceConfig = FirestoreApiKeyConfig.huggingFace();
    remoteAssistantCache = FirestoreRemoteAssistantCache(
      authService: authService,
    );
    assistantRequests = FirestoreAssistantRequests(authService: authService);
    telemetryService = TelemetryService();
    ...
    desktopAuth.onSessionEnded = (uid) async {
      await firestore_cache_factory.clearDesktopFirestoreCacheFor(uid);
      await McpOAuthService.shared.disconnect();
    };
    ...
    accuracyMappingService = DesktopAccuracyMappingService(
      firestore: restFirestore,
    );
    ...
    pitShiftMirrorSyncService = DesktopPitShiftMirrorSyncService(
      authService: desktopAuth,
      firestore: restFirestore,
    );
    ...
    huggingFaceConfig = FirestoreApiKeyConfig.huggingFace(
      remoteFetcher: () async {
        final doc = await restFirestore.getDocument('appConfig/apiKeys');
        final value = doc?.fields['huggingface'];
        return value is String ? value : null;
      },
    );
    match13Config = FirestoreApiKeyConfig.match13(
      remoteFetcher: () async {
        final doc = await restFirestore.getDocument('appConfig/apiKeys');
        final value = doc?.fields['match13'];
        return value is String ? value : null;
      },
    );
    ...
    remoteChatStore = DesktopRemoteChatStore(
      authService: desktopAuth,
      firestore: restFirestore,
    );
    ...
    assistantRequests = DesktopAssistantRequests(
      authService: desktopAuth,
      firestore: restFirestore,
    );
  } else {
    ...
    pitShiftMirrorSyncService = LocalOnlyPitShiftMirrorSyncService();
  ...
  final assistantToolRegistry = AssistantToolRegistry([
    AppDataToolProvider(
      ...
      scoutConfig: configController,
      pickLists: pickListController,
      postMatchReports: postMatchReportController,
    ),
    FrcApiToolProvider(
      tba: tbaClient,
    ...
    if (_isDesktop || _isApple)
      McpAssistantToolProvider(
        baseUrl: McpOAuthService.baseUrlDefault,
        oauth: McpOAuthService.shared,
      ),
  ]);
  ...
  final assistantService = assistantConfig == null
      ...
      : AssistantService(
          backends: [
            ...localAssistantBackends(assistantToolRegistry),
          ...
          shared: assistantRequests,
        );

  final chatBackends = localAssistantBackends(assistantToolRegistry);
  final chatAssistantService = chatBackends.isEmpty
      ? null
      : AssistantService(
  ...
  final requestQueue = assistantRequests;
  if (chatAssistantService != null && requestQueue != null) {
    final worker = AssistantRequestWorker(
    ...
    worker.start();
    if (!_isDesktop) {
      AppLifecycleListener(onResume: worker.start, onHide: worker.stop);
    }
  }

  if (_isDesktop) {
    LlamaRuntimeService.shared.huggingFaceTokenLoader =
        huggingFaceConfig?.teamKey;
  }
  ...
  final pitShiftMirrorController = PitShiftMirrorController(
    syncService: pitShiftMirrorSyncService,
  );
  ...
  final photoUploadService = PitPhotoUploadService(
    baseUrlLoader: photoWorkerConfig.resolve,
    idTokenProvider: authService.idToken,
  );
  issueReportService = IssueReportService(
    write: reportWriter,
    uploadScreenshot: photoUploadService.upload,
  );
  ...
  runApp(
    StrategyApp(
      ...
      pitShiftMirrorController: pitShiftMirrorController,
      ...
      assistantChatController: chatAssistantService == null
          ? null
          : AssistantChatController(
              assistant: chatAssistantService,
              storage: SharedPreferencesAssistantChatStore(),
              remote: remoteChatStore,
            ),
      teamHistory: teamHistoryService,
    ),
  );
