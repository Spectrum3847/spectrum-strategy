...
void main() {
  ...
  group('TrexTraitReport', () {
    ...
    test('fromJson defaults eventKey to empty on a document from before it existed', () {
      final decoded = TrexTraitReport.fromJson(const {
        'trait': 'defense',
        'teamNumber': 254,
        'matchNumber': 1,
      });
      expect(decoded.eventKey, isEmpty);
    });
    ...
    test('copyWith stamps eventKey without touching the rest', () {
      final base = TrexTraitReport(
        id: 'r1',
        trait: TrexTrait.driverSkill.key,
        teamNumber: 254,
      ...
      final stamped = base.copyWith(eventKey: '2026miket');
      expect(stamped.eventKey, '2026miket');
      expect(stamped.report, base.report);
      expect(stamped.id, base.id);
    });
  });
