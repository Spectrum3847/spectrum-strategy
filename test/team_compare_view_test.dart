...
void main() {
  ...
  testWidgets('a long choice label renders in full, not truncated', (
    tester,
  ) async {
    final scouting = await _seed([
      _entry(254, tbaMatchKey: '2026txhou_qm1', fieldValues: {'auLow': 'N/A'}),
    ]);
    await tester.pumpWidget(
      _host(scoutingController: scouting, configController: await _config()),
    );
    ...
    final label = tester.widget<Text>(find.text('Not Attempted'));
    expect(label.overflow, isNot(TextOverflow.ellipsis));
    expect(tester.takeException(), isNull);
  });

  testWidgets('the fuel graph handles non-contiguous match numbers', (
    tester,
  ) async {
    final scouting = await _seed([
      _entry(
      ...
      _entry(
        254,
        tbaMatchKey: '2026txhou_qm4',
        fieldValues: {'autoFuelScored': 8, 'teleopFuelScored': 12},
      ),
    ]);
    await tester.pumpWidget(
      _host(scoutingController: scouting, configController: await _config()),
  ...
  test('fuelChartXLabel prefers the real match number over row position', () {
    expect(fuelChartXLabel(0, const [1, 4]), '1');
    expect(fuelChartXLabel(1, const [1, 4]), '4');
    expect(fuelChartXLabel(1, const [1, null]), '2');
  });
