...
void main() {
  ...
  group('PitScoutingController (local-only)', () {
    ...
    test(
      ...
      () async {
        ...
        final stored = reloaded.firstWhere((e) => e.id == 'doc1');
        expect(
          stored.fieldValues['fieldMap'],
          'a1b2c3d4-e5f6-7890-abcd-ef0123456789.pdf',
        );
      },
    );
    ...
    test('editing an entry twice lands both writes in call order through the '
        'save queue', () async {
      await controller.bootstrap();
      final entry = PitScoutEntry(
        id: 'edit1',
        teamNumber: 3847,
        fieldValues: const {'notes': 'first'},
      );
      await controller.saveEntry(entry);

      final firstEdit = controller.saveEntry(
        entry.copyWith(fieldValues: const {'notes': 'second'}),
      );
      final secondEdit = controller.saveEntry(
        entry.copyWith(fieldValues: const {'notes': 'third'}),
      );
      await firstEdit;
      await secondEdit;

      expect(controller.entries, hasLength(1));
      expect(controller.entries.single.fieldValues['notes'], 'third');
      expect(storage.data['edit1']!.fieldValues['notes'], 'third');
    });
  });
  ...
  group('PitScoutingController sync', () {
    ...
    test('editing an entry pushes the update to sync service', () async {
      await controller.bootstrap();
      final entry = PitScoutEntry(
        id: 'e1',
        teamNumber: 3847,
      ...
      await controller.saveEntry(
        entry.copyWith(fieldValues: const {'notes': 'corrected'}),
      );
      await Future<void>.delayed(Duration.zero);

      expect(sync.pushed, hasLength(1));
      expect(sync.pushed.single.id, 'e1');
      expect(sync.pushed.single.fieldValues['notes'], 'corrected');
    });
  ...
  group('photo upload', () {
    ...
    test('photoBytes resolves to null, not an exception, when the Worker '
        'request throws', () async {
      ...
      final remote = PitScoutEntry.fromJson(<String, dynamic>{
        'id': 'e3',
        'teamNumber': 254,
        'photoKeys': <String>['key-cors.jpg'],
        'updatedAt': DateTime.utc(2026, 8, 1).toIso8601String(),
      });

      expect(await controller.photoBytes(remote, 'key-cors.jpg'), isNull);
    });
