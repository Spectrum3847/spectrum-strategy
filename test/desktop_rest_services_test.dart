...
import 'package:spectrumstrategy/src/scouting/services/desktop_accuracy_mapping_service.dart';
...
void main() {
  ...
  group('DesktopAccuracyMappingService', () {
    test('load reads appConfig/accuracyMapping over REST', () async {
      late Uri url;
      final service = DesktopAccuracyMappingService(
        firestore: _firestore(
          MockClient((request) async {
            url = request.url;
            return http.Response(
              _doc('appConfig/accuracyMapping', {
                'year': 2026,
      ...
      final mapping = await service.load();

      expect(url.path, endsWith('appConfig/accuracyMapping'));
      expect(mapping?['year'], 2026);
      expect(mapping?['perFieldTolerancePct'], 40);
    });

    test('load returns null for a document that does not exist yet', () async {
      final service = DesktopAccuracyMappingService(
        firestore: _firestore(MockClient((_) async => http.Response('', 404))),
      );

      expect(await service.load(), isNull);
    });

    test('load throws on a permission-denied response', () async {
      final service = DesktopAccuracyMappingService(
        firestore: _firestore(
          MockClient(
            (_) async => http.Response(
      ...
      await expectLater(
        service.load,
        throwsA(
          isA<fc.FirestoreApiException>().having(
            (e) => e.status,
            'status',
            'PERMISSION_DENIED',
          ),
    ...
    test('save writes the mapping to appConfig/accuracyMapping', () async {
      late Uri url;
      late Map<String, dynamic> written;
      final service = DesktopAccuracyMappingService(
        firestore: _firestore(
          MockClient((request) async {
            ...
            written = ((jsonDecode(request.body) as Map)['fields'] as Map)
                .cast<String, dynamic>();
            return http.Response(_doc('appConfig/accuracyMapping', {}), 200);
          }),
        ),
      );

      await service.save(<String, dynamic>{'year': 2027, 'minWrongFields': 4});

      expect(url.path, endsWith('appConfig/accuracyMapping'));
      final decoded = fc.FirestoreValueCodec.decodeFields(written);
      expect(decoded['year'], 2027);
      expect(decoded['minWrongFields'], 4);
    });
  });
