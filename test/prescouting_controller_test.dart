...
void main() {
  ...
  group('PrescoutingController sync', () {
    ...
    test('a push the server rejects reports rejected, not offline', () async {
      ...
      await controller.bootstrap();
      await controller.saveEntry(PrescoutEntry(id: 'e1', teamNumber: 3847));
      await Future<void>.delayed(Duration.zero);

      expect(sync.pushed, isEmpty);
      expect(controller.syncStatus.state, PrescoutingSyncState.rejected);
    });
    ...
    test('a 401-shaped failure still reports offline, not rejected', () async {
      ...
      expect(sync.pushed, isEmpty);
      expect(controller.syncStatus.state, PrescoutingSyncState.offline);
    });

    test('a denied read reports noAccess', () async {
      await controller.bootstrap();
      sync.emitStatus(
        const PrescoutingSyncStatus(state: PrescoutingSyncState.noAccess),
      );
      await Future<void>.delayed(Duration.zero);

      expect(controller.syncStatus.state, PrescoutingSyncState.noAccess);
    });
    ...
    test('a rejected entry stops being retried once _repushUnsynced has seen '
      ...
      sync.emitStatus(
        const PrescoutingSyncStatus(state: PrescoutingSyncState.offline),
      );
      await Future<void>.delayed(Duration.zero);
      sync.emitStatus(
        const PrescoutingSyncStatus(state: PrescoutingSyncState.synced),
      );
      await Future<void>.delayed(Duration.zero);
      expect(sync.pushed, isEmpty);
    ...
    test('notifies once the signed-in uid resolves during initialize(), even '
      ...
      storage.data['own'] = PrescoutEntry(
        ...
        teamNumber: 254,
        authorUid: 'uid-me',
        authorDisplayName: 'Matthieu',
      );
      final raceSync = FakePrescoutingSyncService(
        initialState: PrescoutingSyncState.synced,
        uidAfterInitialize: 'uid-me',
      );
      final raceController = PrescoutingController(
        storage: storage,
        syncService: raceSync,
      );
      addTearDown(raceController.dispose);

      var notifyCount = 0;
      raceController.addListener(() => notifyCount++);
      expect(raceController.currentUserUid, isNull);

      await raceController.bootstrap();

      expect(raceController.currentUserUid, 'uid-me');
      expect(notifyCount, greaterThan(0));

      final own = raceController.entries.single;
      expect(own.authorUid, raceController.currentUserUid);
    });
  });
}
