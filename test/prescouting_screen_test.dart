...
void main() {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        tester
            .widget<TextField>(
              find.byKey(const ValueKey('prescout-field-watchedEvent')),
            )
            .controller!
            .text,
        isEmpty,
  ...
  testWidgets('a strategy lead can edit and delete a teammate\'s record', (
    tester,
  ) async {
    final storage = FakePrescoutingStorage();
    await storage.saveEntry(
    ...
    await tester.tap(find.text('Teammate').last);
    await tester.pumpAndSettle();
    expect(find.text('Scouted by'), findsNothing);
    await _tapSaveRecord(tester);
  ...
  for (final (state, chipLabel) in <(PrescoutingSyncState, String)>[
    (PrescoutingSyncState.signedOut, 'Not signed in to sync'),
    (PrescoutingSyncState.noAccess, 'No team access yet'),
  ]) {
    testWidgets(
      ...
      (tester) async {
        ...
        final sync = FakePrescoutingSyncService(initialState: state);
        final controller = await _bootController(storage: storage, sync: sync);
        await _pumpScreen(tester, controller: controller);
        await _openDataTab(tester);
        await _selectEvent(tester);
        ...
        final expectedLocks = state == PrescoutingSyncState.noAccess ? 2 : 1;
        expect(
          find.byIcon(Icons.lock_outline_rounded),
          findsNWidgets(expectedLocks),
        );
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(saved['autoFuelScored'], 14);
      expect(saved['teleopFuelScored'], 53);
      expect(saved['comments'], 'Clean auto.');
    },
  );
  ...
  testWidgets('a side swipe leaves the tab alone on a desktop OS', (
    tester,
  ) async {
    ...
    try {
      ...
      expect(find.textContaining('Pick any robot'), findsOneWidget);
    } finally {
      debugDefaultTargetPlatformOverride = null;
    }
  });

  testWidgets('a side swipe still changes tabs on a touch OS', (tester) async {
    ...
    try {
      ...
      expect(find.textContaining('Pick any robot'), findsNothing);
      expect(
        find.byKey(const ValueKey('prescout-event-dropdown')),
        findsOneWidget,
      );
    } finally {
      debugDefaultTargetPlatformOverride = null;
