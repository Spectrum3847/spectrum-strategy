...
void main() {
  ...
  test(
    'a total Statbotics outage on a never-loaded event still degrades to TBA',
    () async {
      final controller = controllerWith(
      ...
      expect(controller.dataNotice, contains('Statbotics is unavailable'));
      expect(controller.dataNotice, contains('The Blue Alliance'));
      expect(controller.dataNotice, contains('EPA stats return'));
    },
  );
  ...
  test(
    'the event picker also degrades to TBA during a total Statbotics outage',
    () async {
      final controller = controllerWith(
        (request) async => http.Response('{}', 500),
        tbaHandler: healthyTba,
      ...
      expect(controller.availableEvents, isNotEmpty);
      expect(controller.availableEvents.single.key, '2026txhou');
    },
  );

  test(
    'team names fall back to the on-device cache when TBA itself errors too',
    () async {
      ...
      final controller = controllerWith(
        (request) async => http.Response('{}', 500),
        tbaHandler: (request) async {
          if (request.url.path.endsWith('/teams/simple')) {
            return http.Response('server error', 500);
          }
          return healthyTba(request);
        },
        clock: () => fetchedAt.add(const Duration(minutes: 10)),
      );
      await controller.setEventKey('2026txhou');
  ...
  test(
    'the three TBA fallback calls run concurrently, not one after another',
    () async {
      final started = <String>{};
      final barrier = Completer<void>();
      Future<http.Response> gatedTba(http.Request request) async {
        final path = request.url.path;
        String? which;
        if (path.endsWith('/matches/simple')) {
          which = 'schedule';
        } else if (path.endsWith('/teams/simple')) {
          which = 'names';
        } else if (path.endsWith('/event/2026txhou')) {
          which = 'event';
        }
        if (which != null) {
          started.add(which);
          if (started.length == 3 && !barrier.isCompleted) {
            barrier.complete();
          }
      ...
      await controller
          .setEventKey('2026txhou')
          .timeout(const Duration(seconds: 5));

      expect(started, {'schedule', 'names', 'event'});
      expect(controller.matches, hasLength(1));
      expect(controller.eventName, 'Houston');
      expect(controller.error, isNull);
    },
  ...
  Future<http.Response> statboticsScheduleNetworkError(
  ...
  test(
    'a TBA error during the schedule fallback reports its own cause',
    () async {
      final controller = controllerWith(
        statboticsScheduleNetworkError,
        tbaHandler: (request) async {
          if (request.url.path.endsWith('/matches/simple')) {
            return http.Response('service unavailable', 500);
          }
          return healthyTba(request);
        },
      );
      ...
      expect(controller.error, contains('match schedule'));
      expect(controller.error, contains('TBA is busy (500)'));
      expect(controller.error, isNot(contains('network error')));
      expect(controller.matches, isEmpty);
    },
  );

  test(
    'TBA answering with an empty schedule is a notice, not an error',
    () async {
      final controller = controllerWith(
        statboticsScheduleNetworkError,
        tbaHandler: (request) async {
      ...
      expect(
        ...
        contains('No match schedule has been posted for this event yet.'),
      );
    },
  );

  test(
    'TBA succeeding after a lone Statbotics schedule failure clears the error',
    () async {
      final controller = controllerWith(
        statboticsScheduleNetworkError,
        tbaHandler: healthyTba,
  ...
  test(
    'paints cached event data immediately, then replaces it in place',
    () async {
      final fetchedAt = DateTime.utc(2026, 7, 9, 8);
      final warm = controllerWith(healthyApi, clock: () => fetchedAt);
      await warm.setEventKey('2026txhou');
      ...
      final controller = controllerWith(
        (_) => gate.future,
        clock: () => fetchedAt.add(const Duration(hours: 2)),
      );

      final pending = controller.setEventKey('2026txhou');

      for (var i = 0; i < 20 && controller.teamEvents.isEmpty; i++) {
        await Future<void>.delayed(Duration.zero);
      }
      ...
      expect(controller.dataNotice, contains('cached event data'));
      expect(controller.dataNotice, contains('2 hours ago'));

      gate.complete(http.Response('overloaded', 500));
      await pending;

      expect(controller.dataNotice, isNot(contains('while the latest loads')));
    },
  );

  test(
    'switching events mid-fetch drops the stale fetch instead of applying it',
    () async {
      final gateHou = Completer<http.Response>();
      final controller = EventController(
        client: StatboticsClient(
          httpClient: MockClient((request) async {
            if (request.url.toString().contains('2026txhou')) {
              return gateHou.future;
            }
            return statboticsWithoutTheEvent(request);
          }),
          sleep: (_) async {},
        ),
      );
      ...
      gateHou.complete(http.Response(eventJson, 200));
      await first;
  ...
  test(
    'switching events during TBA recovery drops the stale recovery too',
    () async {
      final gateTba = Completer<http.Response>();
      var houAskedTba = false;
      final controller = controllerWith(
        (request) async {
          if (request.url.toString().contains('2026txhou')) {
            return http.Response('overloaded', 500);
        ...
        tbaHandler: (request) async {
          if (request.url.path.contains('2026txhou')) {
            houAskedTba = true;
            return gateTba.future;
          }
          return tbaWithTheEvent(request);
        },
      );
      ...
      for (var i = 0; i < 200 && !houAskedTba; i++) {
        await Future<void>.delayed(Duration.zero);
      }
      expect(houAskedTba, isTrue);
      expect(controller.isLoading, isTrue);

      await controller.setEventKey('2026txdri1');
      expect(controller.matches.single.key, '2026txdri1_qm1');
      expect(controller.isLoading, isFalse);

      gateTba.complete(http.Response(tbaMatchesJson, 200));
      await first;
  ...
  test(
    'paints the cached event list immediately, then replaces it in place',
    () async {
      final warm = controllerWith(statboticsEventList);
      await warm.loadEventsForYear(2026);
      expect(warm.availableEvents, hasLength(1));

      final gate = Completer<http.Response>();
      final controller = controllerWith((_) => gate.future);

      final pending = controller.loadEventsForYear(2026);
      for (var i = 0; i < 20 && controller.availableEvents.isEmpty; i++) {
        await Future<void>.delayed(Duration.zero);
      }

      expect(controller.eventsLoading, isTrue);
      expect(controller.availableEvents, hasLength(1));
      expect(controller.availableEvents.single.key, '2026txhou');

      gate.complete(http.Response('overloaded', 503));
      await pending;
  ...
  test('switching years mid-fetch does not let the stale year win', () async {
    final gate = Completer<http.Response>();
    final controller = EventController(
      client: StatboticsClient(
        httpClient: MockClient((request) async {
          if (request.url.queryParameters['year'] == '2025') {
            return gate.future;
          }
          return statboticsEventList(request);
        }),
        sleep: (_) async {},
      ),
    );

    final first = controller.loadEventsForYear(2025);
    await Future<void>.delayed(Duration.zero);
    expect(controller.eventsLoading, isTrue);

    await controller.loadEventsForYear(2026);
    expect(controller.availableEvents.single.key, '2026txhou');
    expect(controller.eventsLoading, isFalse);
  ...
  group('match13 ratings', () {
    StatboticsEpa xp(double total, {double? auto, double? teleop}) =>
        StatboticsEpa(
          totalPoints: total,
          autoPoints: auto,
          teleopPoints: teleop,
    ...
    test('replaces the ratings and keeps the rank, record and name', () async {
      final controller = controllerWith(
        healthyApi,
        match13: FakeMatch13RatingsService.forEvent('2026txhou', {
          3847: xp(81.5, auto: 19, teleop: 54.25),
        }),
      );

      await controller.setEventKey('2026txhou');

      expect(controller.ratingsSource, RatingsSource.match13);
      final team = controller.teamEvents.single;
      expect(team.epa.totalPoints, 81.5);
      expect(team.epa.autoPoints, 19);
      expect(
        team.teamName,
        'Spectrum',
        reason:
            'match13 carries no team names, so Statbotics keeps supplying '
            'them',
      );
      expect(team.rank, 1, reason: 'and no qualification rank');
      expect(team.wins, 1, reason: 'and no win-loss record');
      expect(
        controller.dataNotice,
        contains('match13 xP'),
        reason:
            'the column is labelled EPA, so the screen says which model '
            'the number is',
      );
    });

    test('rates a team Statbotics did not return at all', () async {
      final controller = controllerWith(
        healthyApi,
        match13: FakeMatch13RatingsService.forEvent('2026txhou', {
          3847: xp(81.5),
      ...
      expect(controller.teamEvents, hasLength(2));
      final added = controller.teamEvents.firstWhere((t) => t.team == 118);
      expect(added.epa.totalPoints, 110.75);
      expect(added.event, '2026txhou');
      expect(added.year, 2026);
      expect(
        added.rank,
        isNull,
        reason: 'nothing here knows a rank for a team Statbotics did not send',
      );
    });

    test('leaves everything alone when match13 cannot answer', () async {
      final controller = controllerWith(
        healthyApi,
        match13: FakeMatch13RatingsService.forEvent('2026txhou', null),
      );

      await controller.setEventKey('2026txhou');

      expect(controller.ratingsSource, RatingsSource.statbotics);
      expect(controller.teamEvents.single.epa.totalPoints, isNull);
      expect(
        controller.dataNotice,
        isNull,
        reason: 'a source that was never reachable is not news',
      );
    });

    test('carries an event Statbotics has nothing on', () async {
      Future<http.Response> emptyStatbotics(http.Request request) async {
        if (request.url.path.endsWith('/event/2026txdri1')) {
          return http.Response('server error', 500);
        }
        return http.Response('[]', 200);
      ...
      final controller = controllerWith(
        emptyStatbotics,
        match13: FakeMatch13RatingsService.forEvent('2026txdri1', {
          3847: xp(42.0),
        }),
      );
      ...
      expect(controller.ratingsSource, RatingsSource.match13);
      expect(controller.teamEvents.single.team, 3847);
      expect(controller.teamEvents.single.epa.totalPoints, 42.0);
      expect(controller.dataNotice, contains('match13 xP'));
      expect(
        controller.teamsAreRosterOnly,
        isFalse,
        reason: 'there are ratings to sort on, so this is not a bare roster',
      );
    });

    test('rates a Statbotics outage that TBA covered the rest of', () async {
      final controller = controllerWith(
        (request) async => http.Response('down', 503),
        tbaHandler: healthyTba,
        match13: FakeMatch13RatingsService.forEvent('2026txhou', {
      ...
      expect(
        controller.dataNotice,
        contains('match13 xP'),
        reason: 'this is the outage the second source exists for',
      );
      expect(
        controller.dataNotice,
        isNot(contains('EPA stats return when Statbotics recovers')),
        reason:
            'the ratings are already on screen, so there is nothing to '
            'wait for',
      );
      expect(controller.matches, hasLength(1), reason: 'TBA still has these');
      expect(controller.teamNicknames[3847], 'Spectrum');
    });

    test('a stale cache does not overwrite fresh match13 ratings', () async {
      final warm = controllerWith(healthyApi);
      await warm.setEventKey('2026txhou');
      expect(warm.teamEvents.single.epa.totalPoints, isNull);
      ...
      expect(controller.ratingsSource, RatingsSource.match13);
      expect(
        controller.teamEvents.single.epa.totalPoints,
        81.5,
        reason:
            'the cache fills a part nothing else supplied; match13 '
            'supplied this one',
      );
      expect(controller.dataNotice, contains('match13 xP'));
      expect(
        controller.dataNotice,
        isNot(contains('EPA stats are cached')),
        reason: 'nothing on screen came from the cached ratings',
      );
    });

    test('no service at all behaves exactly as before', () async {
      final controller = controllerWith(healthyApi);
