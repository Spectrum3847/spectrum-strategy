...
void main() {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await tester.enterText(find.byKey(const ValueKey('alliance-0-0')), '254');
      await boardController.pendingWrites;
      await tester.pump(Duration.zero);
      ...
      final panels = tester
          .widgetList<Text>(
            find.descendant(
              of: find.byKey(const ValueKey('next-up-panels')),
              matching: find.byType(Text),
            ),
          )
          .map((t) => t.data)
          .toList();
      expect(panels, isNot(contains('254')));

      expect(panels, contains('2056'));
    },
  );
  ...
  testWidgets('a carded team is highlighted in the alliance grid', (
    ...
    final boardController = PlayoffBoardController(
      storage: FakePlayoffBoardStorage(<String, PlayoffBoard>{
        '2026txhou': const PlayoffBoard(
          allianceCells: <String, String>{'0,0': '118'},
        ),
      }),
    );
    await boardController.bootstrap();
    ...
    final cell = tester.widget<Container>(
      find
          .ancestor(
            of: find.byKey(const ValueKey('alliance-0-0')),
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await configController.bootstrap();
      final scoutingController = await _seed([
        _entry(118, match: 'qf1', fieldValues: {'ryCard': 'Yellow'}),
        _entry(254, match: 'qf1', fieldValues: {'ryCard': 'Red'}),
      ]);
      final boardController = PlayoffBoardController(
        storage: FakePlayoffBoardStorage(<String, PlayoffBoard>{
          '2026txhou': const PlayoffBoard(
            allianceCells: <String, String>{'0,0': '118', '0,1': '254'},
          ),
        }),
      );
      await boardController.bootstrap();
      ...
      final context = tester.element(
        find.byKey(const ValueKey('alliance-0-0')),
      );
      expect(colorFor('alliance-0-0'), StrategyPalette.flagWarnOf(context));
      expect(colorFor('alliance-0-1'), StrategyPalette.flagSevereOf(context));
    },
  );
