...
void main() {
  group('writeBoardPng', () {
    test('writes to the injected desktop directory', () async {
      ...
      final service = StrategyExportService(
        exportDirResolver: () async =>
            (directoryPath: tmp.path, description: 'Pictures/SpectrumStrategy'),
      );
      ...
      final written = File('${tmp.path}${Platform.pathSeparator}match_1.png');
      expect(await written.exists(), isTrue);
      expect(result.savedMessage, contains('match_1.png'));
    ...
    test('uses the mobile description when the resolver says so', () async {
      ...
      final service = StrategyExportService(
        exportDirResolver: () async =>
            (directoryPath: tmp.path, description: mobileDescription),
      );
      ...
      expect(result.savedMessage, contains(mobileDescription));
      await tmp.delete(recursive: true);
    });
      ...
      () async {
        ...
        final service = StrategyExportService(
          exportDirResolver: () async => (
            ...
            description: 'Pictures/SpectrumStrategy',
          ),
        );
