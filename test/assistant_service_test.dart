...
void main() {
  ...
  group('key resolution', () {
    test('the team key wins over the compile-time key', () async {
      final config = FirestoreApiKeyConfig(
    ...
    test('a fetched key is mirrored so it survives going offline', () async {
      ...
      expect(
        prefs.getString(FirestoreApiKeyConfig.openRouterPrefsKey),
        'team-key',
      );
    });
    ...
    test('a fetch failure falls back to the mirror', () async {
      SharedPreferences.setMockInitialValues(<String, Object>{
        FirestoreApiKeyConfig.openRouterPrefsKey: 'mirrored',
      });
      final config = FirestoreApiKeyConfig(
    ...
    test('an admin clearing the key clears the mirror too', () async {
      ...
      final config = FirestoreApiKeyConfig(remoteFetcher: () async => '');

      expect(await config.teamKey(), isNull);
      final prefs = await SharedPreferences.getInstance();
      expect(prefs.getString(FirestoreApiKeyConfig.openRouterPrefsKey), isNull);
    });

    test('no key anywhere resolves to null', () async {
      final config = FirestoreApiKeyConfig(remoteFetcher: () async => null);
  ...
  group('OpenRouter backend', () {
    test('sends the router alias and the team key', () async {
      ...
      final backend = OpenRouterAssistantBackend(
        config: FirestoreApiKeyConfig(remoteFetcher: () async => 'k'),
    ...
    test(
      ...
      () async {
        var calls = 0;
        final backend = OpenRouterAssistantBackend(
          config: FirestoreApiKeyConfig(remoteFetcher: () async => 'bad'),
          retryDelay: Duration.zero,
          client: MockClient((_) async {
            calls++;
    ...
    test('is unavailable with no key, and does not call out', () async {
      var called = false;
      final backend = OpenRouterAssistantBackend(
        config: FirestoreApiKeyConfig(remoteFetcher: () async => null),
        client: MockClient((_) async {
          called = true;
  ...
  group('service', () {
    ...
    test('paces concurrent calls to a rationed backend', () async {
      final backend = _RationedRecordingBackend();
      final service = AssistantService(
        backends: [backend],
        cache: AssistantCache(),
    ...
    test('does not pace a backend that costs nothing', () async {
      final backend = _RecordingBackend();
      final service = AssistantService(
        backends: [backend],
        cache: AssistantCache(),
        minimumGap: const Duration(seconds: 30),
      );
    ...
    test(
      'a local answer means the rationed backend is never reached',
      () async {
        final local = _RecordingBackend(source: AssistantSource.apple);
        final router = _RationedRecordingBackend();
        final service = AssistantService(
  ...
  group('backend wiring order', () {
    ...
    test('the llama.cpp runtime answers ahead of the network router, '
        "matching main.dart's desktop backend list", () async {
      ...
      File('${modelsDir.path}/installed_v1.json')
          .writeAsStringSync(jsonEncode({model.id: model.toJson()}));
      ...
      final service = AssistantService(
        backends: [local, unavailableRouter],
        cache: AssistantCache(),
        minimumGap: Duration.zero,
      );
...
void _converseGroup() {
  group('converse', () {
    ...
    test(
      'never writes a conversational answer into the shared cache',
      () async {
        final cache = AssistantCache();
        final service = AssistantService(
          backends: [_RecordingBackend()],
        ...
        await service.converse(request);

        expect(await cache.read(request.cacheKey), isNull);
      },
    );

    test('answers fresh every time instead of reading the cache', () async {
      final cache = AssistantCache();
      final backend = _RecordingBackend();
      final service = AssistantService(
        backends: [backend],
    ...
    test('keeps the rate gate, and a failure does not wedge it', () async {
      final backend = _RecordingBackend()..failWith = 'no backend';
      final service = AssistantService(
        backends: [backend],
        cache: AssistantCache(),
        minimumGap: Duration.zero,
      ...
      backend.failWith = null;
      final summary = await service.converse(request);
...
class _RationedRecordingBackend extends _RecordingBackend
    implements RationedAssistantBackend {
  _RationedRecordingBackend() : super();
}
