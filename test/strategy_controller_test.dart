...
void main() {
  test('StrategyController records drawing and robot placement', () async {
    ...
    controller.importTeams([3847, 2714]);
  ...
  test(
    ...
    () async {
      ...
      controller.importTeams([3847, 2714, 5114]);
      controller.selectPhase(StrategyPhase.auton);
  ...
  test('autoPlaceTeams splits 6 teams across both alliance sides', () async {
    ...
    controller.setAlliance('Blue');
    controller.importTeams([1, 2, 3, 4, 5, 6]);
    controller.selectPhase(StrategyPhase.teleop);
  ...
  group('undo/redo', () {
    test('undo removes the most recently added stroke', () async {
      final controller = StrategyController(directory: FakeMatchDirectory());
      await controller.bootstrap();
    ...
    test('undo removes the most recently placed marker', () async {
      final controller = StrategyController(directory: FakeMatchDirectory());
      await controller.bootstrap();
      controller.importTeams([3847]);
      controller.setSelectedRobotTeam(3847);
      ...
      controller.placeRobot(const StrategyPoint(0.5, 0.6));
      expect(controller.session.markersFor(StrategyPhase.auton), hasLength(1));

      controller.undo();
      expect(controller.session.markersFor(StrategyPhase.auton), isEmpty);

      controller.redo();
      expect(controller.session.markersFor(StrategyPhase.auton), hasLength(1));
      expect(
        controller.session.markersFor(StrategyPhase.auton).first.teamNumber,
        3847,
      );
    });

    test('undo puts a dragged marker back where it started', () async {
      const size = Size(1000, 500);
      final controller = StrategyController(directory: FakeMatchDirectory());
      await controller.bootstrap();
      controller.selectTool(StrategyTool.robot);
      ...
      final grabbed = controller.startMarkerDragAt(
        const StrategyPoint(0.2, 0.2).toOffset(size),
        size,
      );
      expect(grabbed, isTrue);
      controller.updateMarkerDrag(const StrategyPoint(0.8, 0.8));
      controller.finishMarkerDrag();
      expect(
        controller.session.markersFor(StrategyPhase.auton).first.position.x,
        0.8,
      );
    ...
    test('a multi-element erase drag undoes in one step', () async {
      const size = Size(1000, 500);
      final controller = StrategyController(directory: FakeMatchDirectory());
      await controller.bootstrap();
      ...
      controller.startStroke(const StrategyPoint(0.9, 0.9));
      controller.finishStroke();
      expect(controller.session.strokesFor(StrategyPhase.auton), hasLength(2));

      controller.selectTool(StrategyTool.delete);
      expect(
        controller.eraseAt(const StrategyPoint(0.1, 0.1).toOffset(size), size),
        isTrue,
      );
      expect(
        controller.eraseAt(const StrategyPoint(0.9, 0.9).toOffset(size), size),
        isTrue,
      );
    ...
    test('rapid note edits coalesce into one undo step', () async {
      final controller = StrategyController(directory: FakeMatchDirectory());
      await controller.bootstrap();

      controller.updateNote('p');
      controller.updateNote('pr');
      controller.updateNote('pre');
      expect(controller.session.noteFor(StrategyPhase.auton), 'pre');

      controller.undo();
      expect(controller.session.noteFor(StrategyPhase.auton), isEmpty);
      expect(controller.canUndo, isFalse);
    });

    test('undo respects phase boundaries', () async {
      final controller = StrategyController(directory: FakeMatchDirectory());
      await controller.bootstrap();
    ...
    test('a new action clears redo history', () async {
      final controller = StrategyController(directory: FakeMatchDirectory());
      await controller.bootstrap();
      ...
      controller.startStroke(const StrategyPoint(0.3, 0.4));
      controller.finishStroke();
      expect(controller.canRedo, isFalse);
    });

    test('undo history does not carry over to a new match', () async {
      final controller = StrategyController(directory: FakeMatchDirectory());
      await controller.bootstrap();
    ...
    test('undo goes through the save queue like any other edit', () async {
      final directory = FakeMatchDirectory();
      final controller = StrategyController(directory: directory);
      await controller.bootstrap();
      ...
      final relaunched = StrategyController(directory: directory);
      await relaunched.bootstrap();
      expect(relaunched.session.strokesFor(StrategyPhase.auton), isEmpty);
    });
  });
}
