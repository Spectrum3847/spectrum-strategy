...
void main() {
    ...
    test('writes the CSV to the injected export directory', () async {
      ...
      final service = ScoutExportService(
        exportDirResolver: () async => (
          directory: tmp.path,
          description: 'Documents/SpectrumStrategy/exports',
        ),
      );
      ...
      expect(result.filePath, contains('2026_test_event_scouting_export.csv'));
      expect(File(result.filePath).parent.path, tmp.path);
  ...
  group('ScoutExportService.buildSummaryCsv', () {
    final service = ScoutExportService();

    test('header matches the Summary table columns in order', () {
      final csv = service.buildSummaryCsv(rows: const [], eventKey: '2026test');

      final header = csv.split('\r\n').first;
      expect(
        header,
        'Event,Team,IQM Teleop,Max Teleop,IQM Auto,Max Auto,Auto Climb,'
        'Low Climb,Mid Climb,High Climb',
      );
    });

    test('formats stat columns bare and rate columns as a percentage', () {
      const row = TeamSummaryRow(
        teamNumber: 3847,
        iqmTeleop: 12,
        maxTeleop: 18.5,
        iqmAuto: 4,
      ...
      final csv = service.buildSummaryCsv(rows: [row], eventKey: '2026test');

      final dataRow = csv.split('\r\n')[1];
      expect(dataRow, '2026test,3847,12,18.5,4,6,75%,50%,100%,0%');
    });

    test('leaves a cell blank for a team with no value in that column', () {
      const row = TeamSummaryRow(teamNumber: 254);

      final csv = service.buildSummaryCsv(rows: [row], eventKey: '2026test');

      final dataRow = csv.split('\r\n')[1];
      expect(dataRow, '2026test,254,,,,,,,,');
    });
  });
}
