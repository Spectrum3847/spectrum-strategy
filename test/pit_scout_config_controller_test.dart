...
''';
  ...
  test('the Notes section renders as long text, not a one-line box', () async {
    final config = await ScoutConfigService.pit().loadDefault();
    final notes = config.sections.firstWhere((s) => s.name == 'Notes');
    for (final field in notes.fields) {
      expect(
        field.type,
    ...
    final launcherType = config.allFields.firstWhere(
      (f) => f.code == 'launcherType',
    );
    expect(launcherType.type, ScoutFieldType.text);
  });
  ...
  test(
    ...
    () async {
      ...
      final fieldMap = fieldMapSection.fields.firstWhere(
        (f) => f.code == 'fieldMap',
        orElse: () => throw StateError('no fieldMap field in Field Map'),
      );
      expect(fieldMap.type, ScoutFieldType.document);
      expect(fieldMap.required, isFalse);
      expect(
        fieldMap.referenceImageAsset,
