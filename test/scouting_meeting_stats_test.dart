...
const _ryCard = ScoutConfigField(
  ...
  code: 'ryCard',
  choices: <String, String>{'None': 'None', 'Yellow': 'Yellow', 'Red': 'Red'},
);
...
void main() {
  ...
  group('ScoutingMeetingStats.cardedTeams', () {
    test('collects distinct teams with a card entry, legacy or current', () {
      final entries = [
        _entry(1, match: 'qm1', fieldValues: {'ryCard': 'Red'}),
        _entry(1, match: 'qm5', fieldValues: {'ryCard': 'None'}),

        _entry(2, match: 'qm2', fieldValues: {'ryCard': true}),
        _entry(3, match: 'qm3', fieldValues: {'ryCard': 'true'}),
        _entry(4, match: 'qm4', fieldValues: {'ryCard': false}),
        _entry(5, match: 'qm5', fieldValues: {}),
      ];

      expect(ScoutingMeetingStats.cardedTeams(entries), [1, 2, 3]);
    });
  ...
  group('ScoutingMeetingStats.cardedTeamColors', () {
    test('distinguishes yellow from red', () {
      final entries = [
        _entry(1, match: 'qm1', fieldValues: {'ryCard': 'Yellow'}),
        _entry(2, match: 'qm2', fieldValues: {'ryCard': 'Red'}),
        _entry(3, match: 'qm3', fieldValues: {'ryCard': 'None'}),
      ];

      expect(ScoutingMeetingStats.cardedTeamColors(entries), {
        1: CardColor.yellow,
        2: CardColor.red,
      });
    });

    test('a card recorded without a color reads as unknown, not red', () {
      final entries = [
    ...
    test('red wins when the same team has both colors across matches', () {
      final entries = [
        _entry(1, match: 'qm1', fieldValues: {'ryCard': 'Yellow'}),
        _entry(1, match: 'qm2', fieldValues: {'ryCard': 'Red'}),
      ];
    ...
    test(
      'severity ranks red above unknown above yellow, either match order',
      () {
        final redThenUnknown = [
          _entry(1, match: 'qm1', fieldValues: {'ryCard': 'Red'}),
          _entry(1, match: 'qm2', fieldValues: {'ryCard': true}),
        ];
        final unknownThenRed = [
          _entry(2, match: 'qm1', fieldValues: {'ryCard': true}),
          _entry(2, match: 'qm2', fieldValues: {'ryCard': 'Red'}),
        ];
        final yellowThenUnknown = [
          _entry(3, match: 'qm1', fieldValues: {'ryCard': 'Yellow'}),
          _entry(3, match: 'qm2', fieldValues: {'ryCard': true}),
        ];
        final unknownThenYellow = [
          _entry(4, match: 'qm1', fieldValues: {'ryCard': true}),
          _entry(4, match: 'qm2', fieldValues: {'ryCard': 'Yellow'}),
        ];

        expect(
          ScoutingMeetingStats.cardedTeamColors(redThenUnknown),
          containsPair(1, CardColor.red),
        );
        expect(
          ScoutingMeetingStats.cardedTeamColors(unknownThenRed),
          containsPair(2, CardColor.red),
        );
        expect(
          ScoutingMeetingStats.cardedTeamColors(yellowThenUnknown),
          containsPair(3, CardColor.unknown),
        );
        expect(
          ScoutingMeetingStats.cardedTeamColors(unknownThenYellow),
          containsPair(4, CardColor.unknown),
        );
      },
    );

    test('empty when nobody is carded', () {
      expect(ScoutingMeetingStats.cardedTeamColors(const []), isEmpty);
    });
  });
}
