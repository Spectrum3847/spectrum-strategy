...
void main() {
  ...
  test(
    'editCells applies every override in one schedule and pushes once',
    () async {
      final sync = FakeScoutShiftSyncService(
        uid: 'admin1',
        displayName: 'Admin',
      ...
      await controller.editCells(
        cells: const [(0, 1), (0, 2), (1, 1)],
        text: 'covering',
        color: ScheduleCellColor.grey,
      );

      expect(controller.schedule!.colorFor(0, 1), ScheduleCellColor.grey);
      expect(controller.schedule!.colorFor(0, 2), ScheduleCellColor.grey);
      expect(controller.schedule!.colorFor(1, 1), ScheduleCellColor.grey);
      expect(controller.schedule!.textFor(0, 1), 'covering');
      expect(controller.schedule!.textFor(0, 2), 'covering');
      expect(controller.schedule!.textFor(1, 1), 'covering');

      expect(sync.pushes.length, pushesBefore + 1);
      expect(sync.pushes.last.colorFor(0, 1), ScheduleCellColor.grey);
      expect(sync.pushes.last.colorFor(0, 2), ScheduleCellColor.grey);
      expect(sync.pushes.last.colorFor(1, 1), ScheduleCellColor.grey);
    },
  );

  test('editCells is a no-op with no schedule yet', () async {
    final sync = FakeScoutShiftSyncService(uid: 'admin1', displayName: 'Admin');
    final controller = ScoutShiftController(syncService: sync);
    addTearDown(controller.dispose);
    await controller.watchEvent('2026miket');
  ...
  test(
    'inputNames stores the roster with a blank grid and pushes it',
    () async {
      final sync = FakeScoutShiftSyncService(
        uid: 'admin1',
        displayName: 'Admin',
      ...
      await controller.inputNames(matchCount: 12, roster: _roster(3));

      expect(controller.schedule, isNotNull);
      expect(controller.schedule!.matchCount, 12);
      expect(controller.schedule!.hasRotationShifts, isFalse);
      expect(sync.pushes.single.matchCount, 12);
      expect(sync.pushes.single.hasRotationShifts, isFalse);
    },
  );

  test('inputNames is a no-op with no event selected', () async {
    final sync = FakeScoutShiftSyncService();
    final controller = ScoutShiftController(syncService: sync);
    addTearDown(controller.dispose);

    await controller.inputNames(matchCount: 12, roster: _roster(2));

    expect(controller.schedule, isNull);
    expect(sync.pushes, isEmpty);
  });

  test('generateSchedule builds the rotation for the current schedule and '
      'pushes it', () async {
    final sync = FakeScoutShiftSyncService(uid: 'admin1', displayName: 'Admin');
    final controller = ScoutShiftController(syncService: sync);
    addTearDown(controller.dispose);
    ...
    expect(controller.schedule!.hasRotationShifts, isTrue);
    expect(sync.pushes.last.hasRotationShifts, isTrue);
  });

  test('generateSchedule is a no-op with no schedule yet', () async {
    final sync = FakeScoutShiftSyncService(uid: 'admin1', displayName: 'Admin');
    final controller = ScoutShiftController(syncService: sync);
    addTearDown(controller.dispose);
    await controller.watchEvent('2026miket');
  ...
  test('generateSchedule keeps red overrides and routes around them', () async {
    final sync = FakeScoutShiftSyncService(uid: 'admin1', displayName: 'Admin');
    final controller = ScoutShiftController(syncService: sync);
    addTearDown(controller.dispose);
    await controller.watchEvent('2026miket');
    await controller.inputNames(matchCount: 12, roster: _roster(12));
    await controller.editCell(
      col: 0,
      match: 1,
      text: '',
    ...
    expect(controller.schedule!.colorFor(0, 1), ScheduleCellColor.red);
    expect(controller.schedule!.rotationFor('u0')!.isOnDuty(1), isFalse);
  });
  ...
  testWidgets(
    '"Generate scouting schedule" builds the rotation for a blank grid with '
    'no confirmation',
    (tester) async {
      ...
      await tester.tap(find.text('Generate scouting schedule'));
      await tester.pumpAndSettle();

      expect(find.text('Regenerate the shift rotation?'), findsNothing);
      expect(controller.schedule!.hasRotationShifts, isTrue);
    },
  );

  testWidgets('regenerating an existing rotation asks for confirmation', (
    tester,
  ) async {
    final eventController = await _eventControllerWithMatches();
    addTearDown(eventController.dispose);
    ...
    expect(find.text('Regenerate the shift rotation?'), findsOneWidget);
  ...
  testWidgets(
    'a scouter with no strategy role sees neither shift-building action',
    (tester) async {
      final eventController = await _eventControllerWithMatches();
      addTearDown(eventController.dispose);

      final roleService = FakeUserRoleService();
      roleService.setRole('local', UserRole.scouter);
      final auth = LocalOnlyAuthService();
      final roles = UserRoleController(
        authService: auth,
      ...
      expect(find.text('Input names'), findsNothing);
      expect(find.text('Generate scouting schedule'), findsNothing);
    },
  );
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await tradeSync.respond(
        tradeController.trades.single,
      ...
      expect(
        find.text(
          'Other scouter accepted your trade request -- your shifts '
          'updated.',
        ),
        findsOneWidget,
      );
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        find.textContaining('Other scouter asked you to cover matches 7'),
        findsOneWidget,
      );
      ...
      expect(
        find.text(
          'Other scouter cancelled the trade request for matches 7-12.',
        ),
        findsOneWidget,
      );
    },
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        find.text(
          'You accepted the trade request from Other scouter -- your '
          'shifts updated.',
        ),
        findsOneWidget,
      );
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await tradeController.bootstrap();
      await tradeController.watchEvent('2026miket');
      expect(tradeController.trades.single.isPending, isTrue);
