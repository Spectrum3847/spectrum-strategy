...
void main() {
  ...
  test(
    ...
    () async {
      ...
      expect(snapshots[1].map((r) => r.id).toSet(), {'t1', 't2'});
    },
  );
  ...
  test('delete sends a DELETE request for the report document', () async {
    var sawDelete = false;
    final service = DesktopTrexTraitReportSyncService(
      authService: _signedInAuth(),
      firestore: _firestore(
        MockClient((request) async {
          if (request.method == 'DELETE') {
            sawDelete = true;
            expect(request.url.path, endsWith('trexTraitReports/${report.id}'));
  ...
  test('syncNow retries a delete queued from a previous failure', () async {
    final queue = FakePendingPushQueue();
    await queue.mark('trexTraitReports_deleted', report.id);
