...
void main() {
  ...
  test('the climb level fields offer Not attempted/Failed/Successful, retiring '
    ...
    for (final code in <String>['eLow', 'eMiddle', 'eHigh']) {
      ...
      expect(field.retiredChoiceKeys, <String>{'Outpost', 'Middle', 'Depot'});
      expect(field.activeChoices.keys, <String>['N/A', 'Failed', 'Successful']);
      expect(field.choices, containsPair('Outpost', 'Outpost'));
      expect(field.choices, containsPair('Middle', 'Middle'));
      expect(field.choices, containsPair('Depot', 'Depot'));
    }
  });
  ...
  test(
    ...
    () async {
      ...
      expect(card.type, ScoutFieldType.select);
      expect(card.retiredChoiceKeys, <String>{'true', 'false'});
      expect(card.activeChoices.keys, <String>['None', 'Yellow', 'Red']);
      expect(card.resolveStoredChoice(true), 'true');
      expect(card.resolveStoredChoice(false), 'false');
      expect(card.labelForStored(true), 'Card (color unknown)');
      expect(card.labelForStored(false), 'No card (legacy)');
    },
  );
}
