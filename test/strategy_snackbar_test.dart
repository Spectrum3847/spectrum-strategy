...
void main() {
  ...
  testWidgets('board header does not overflow at phone width', (tester) async {
    ...
    expect(find.text('Erase').hitTestable(), findsOneWidget);
    expect(find.text('Clear phase').hitTestable(), findsOneWidget);
    expect(find.byKey(const ValueKey('strategy-team-legend')), findsOneWidget);
    expect(tester.takeException(), isNull);
    strategy.dispose();
    await tester.pumpAndSettle();
  });
