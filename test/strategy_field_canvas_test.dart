...
void main() {
  ...
  testWidgets('a second pointer mid-stroke does not steal the active stroke', (
    tester,
  ) async {
    final controller = StrategyController(directory: FakeMatchDirectory());
    await controller.bootstrap();
    ...
    final topLeft = tester.getTopLeft(find.byType(StrategyFieldCanvas));

    final stroke1 = await tester.startGesture(topLeft + const Offset(50, 50));
    await tester.pump();
    await stroke1.moveBy(const Offset(20, 0));
    await tester.pump();

    final initial = controller.session.strokesFor(StrategyPhase.auton);
    expect(initial, hasLength(1));
    expect(initial.single.points, hasLength(2));
    expect(initial.single.points.first.x, closeTo(50 / 800, 0.001));

    final stroke2 = await tester.startGesture(topLeft + const Offset(400, 300));
    await tester.pump();
    await stroke2.moveBy(const Offset(20, 0));
    await tester.pump();
    ...
    for (final point in strokes.single.points) {
      expect(point.x, lessThan(0.2));
    }
