...
void main() {
  ...
  group('eventKeyOfEntry', () {
    test('reads the event key off the TBA match key', () {
      expect(
        eventKeyOfEntry(entry(tbaMatchKey: '2026txhou_qm14')),
        '2026txhou',
      );
      expect(
        eventKeyOfEntry(entry(tbaMatchKey: '2026txdri_f1m2')),
        '2026txdri',
      );
    });

    test('an entry with no TBA match key names no event', () {
      expect(eventKeyOfEntry(entry()), isNull);
      expect(eventKeyOfEntry(entry(typedMatchNumber: 'qm12')), isNull);
    });

    test('a malformed TBA key with no underscore names no event', () {
      expect(eventKeyOfEntry(entry(tbaMatchKey: 'garbage')), isNull);
    });
  });
}
