...
void main() {
  ...
  for (final (state, chipLabel) in <(PostMatchReportSyncState, String)>[
    (PostMatchReportSyncState.signedOut, 'Not signed in to sync'),
    (PostMatchReportSyncState.noAccess, 'No team access yet'),
  ]) {
    testWidgets(
      '$state shows the sync chip on the empty table too, so a locked view '
      'reads as a sign-in problem, not a mystery (#1710)',
      (tester) async {
        final controller = PostMatchReportController(
          storage: FakePostMatchReportStorage(),
          syncService: FakePostMatchReportSyncService(initialState: state),
        );
        await controller.bootstrap();
