...
void main() {
  ...
  testWidgets('adding a question appends it to its section', (tester) async {
    ...
    final drivetrainSectionIndex = controller.config.sections.indexWhere(
      (s) => s.name == 'Drivetrain',
    );
    final addQuestion = find.text('Add question').at(drivetrainSectionIndex);
  ...
  testWidgets('a lead can add a document question (#1594)', (tester) async {
    ...
    await tester.tap(find.byType(DropdownButtonFormField<ScoutFieldType>));
    await tester.pumpAndSettle();
    ...
    final field = controller.config.allFields.firstWhere(
      (f) => f.title == 'Field map',
    );
    expect(field.type, ScoutFieldType.document);
  ...
  testWidgets('editing a question keeps its code but changes the title', (
    ...
    await _pump(tester, controller);
    final firstCode = controller.config.allFields.first.code;
    ...
    expect(controller.config.allFields.first.title, 'New Title');
    expect(controller.config.allFields.first.code, firstCode);
  });
  ...
  testWidgets('a non-empty section refuses to delete', (tester) async {
    ...
    expect(controller.config.sections.first.name, 'Scouter');
  });
  ...
  testWidgets('editing a select question shows its current choices', (
    ...
    await _pump(tester, controller);
    final drivetrainIndex = controller.config.allFields.indexWhere(
      (f) => f.code == 'drivetrainType',
    );

    await tester.ensureVisible(
      find.byTooltip('Edit question').at(drivetrainIndex),
    );
    await tester.pumpAndSettle();
    await tester.tap(find.byTooltip('Edit question').at(drivetrainIndex));
    await tester.pumpAndSettle();
  ...
  testWidgets('adding a choice appends it to the select field', (tester) async {
    ...
    final before = controller.config.allFields[drivetrainIndex].choices!.length;
    ...
    final choices = controller.config.allFields[drivetrainIndex].choices!;
  ...
  testWidgets('removing a choice retires it, but never the last active one', (
    ...
    final totalChoices =
        controller.config.allFields[drivetrainIndex].choices!.length;
    ...
    final field = controller.config.allFields[drivetrainIndex];
  ...
  testWidgets('reordering a choice moves it within the choice list', (
    ...
    expect(
      controller.config.allFields[drivetrainIndex].choices!.keys.first,
      'swerve',
    );
  });
