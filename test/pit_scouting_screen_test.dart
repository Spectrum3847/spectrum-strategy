...
void main() {
  ...
  group("editing someone else's pit entry", () {
    Future<PitScoutingController> bootOtherAuthorEntry() async {
      final controller = PitScoutingController(
        storage: FakePitScoutingStorage(),
      );
    ...
    testWidgets('updates the original entry instead of filing a duplicate, and '
        'keeps the original author', (tester) async {
      tester.view.physicalSize = const Size(800, 4000);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.reset);
      ...
      expect(
        find.textContaining("saved entry for this team"),
        findsOneWidget,
        reason:
            'the notice must say whose entry this is, since it is not '
            "the editor's own",
      );
      ...
      final saved = pitScouting.entriesForTeam(3847);
      expect(
        saved,
        hasLength(1),
        reason:
            'an elevated edit must update the original record, not file '
            'a second one under the same team',
      );
      expect(
        saved.single.authorUid,
        'someone-else',
        reason: 'an edit never reassigns authorship',
      );
    });
  });
