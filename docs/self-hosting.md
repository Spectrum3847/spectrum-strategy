...
## Optional: the shift-reminder Worker

Shift-start and shift-trade Slack DMs run as another small Cloudflare Worker,
`scripts/shift-worker/`, rather than a scheduled Node job, so a reminder
lands within minutes of a shift starting instead of once a day. Edit its
`wrangler.jsonc` with your own `account_id`, `FIREBASE_PROJECT`, and
`PLATFORM_PROJECT`, then deploy the same way as the photo Worker:

```
cd scripts/shift-worker
pnpm dlx wrangler@latest deploy
```

It needs `FIREBASE_SERVICE_ACCOUNT`, `SPECTRUM_PLATFORM_SERVICE_ACCOUNT`,
`SLACK_BOT_TOKEN`, and a `RUN_TOKEN` you choose yourself, all set as Worker
secrets with `wrangler secret put`. Something has to call its `POST /run`
route on a schedule; the Cloudflare account's five free cron triggers may
already be spoken for, so an external scheduler (for example cron-job.org)
posting every five minutes with `RUN_TOKEN` in the `X-Run-Token` header works
without needing one.

## Optional: the scheduled jobs

Three Node services in `scripts/` do the work that happens while nobody has
the app open. The app works without all of them, so treat this as a later
...
accuracy mapping from `appConfig/accuracyMapping`.
Run one with `DRY_RUN=1` first; they all honour it.

The Slack one needs a bot token from your own Slack workspace, and it
messages a person by the `slackId` on their profile, so that has to be filled
