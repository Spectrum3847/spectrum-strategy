...
import worker, { run } from './src/index.mjs';
...
test('a second run against the state the first run wrote sends nothing (dedupe)', async () => {
  ...
  const firstStats = await run(first.env);
  ...
  const secondStats = await run(second.env);
...
test('POST /run needs the RUN_TOKEN header and then runs one pass', async () => {
  ...
  assert.ok(body.result && body.result.skipped, 'a run with no secrets is reported as skipped');
});
