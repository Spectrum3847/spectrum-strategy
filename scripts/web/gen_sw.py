...
def build_id(build_dir, template):
    ...
    The worker template is part of the hash: the cache name is what makes a
    new worker start from an empty cache, so a change to the worker's own
    logic has to rotate it too, or the new logic inherits whatever the old
    logic stored.
    """
